...
1. ~~**`blocked_by` 幽灵引用无检出**~~ **✅ 已收口**（后续 PR）——落地方式与原修法方向一致（`validate_state` 加扫描 + 告警，不硬判损坏）。三点落地细化，均非原记账所能预见：
   ...
   - **同批 nit 已修**：三处 rustdoc 内链指向已改名的 `canonicalize_blocked_ref`（实际 `resolve_blocked_ref`；`cargo doc` 报 unresolved link，而 `just ci` **不跑 `cargo doc`** 故 CI 全绿掩盖了它——修完这 3 处后仓内仍有 **5 处** pre-existing 断链（`SubAgentCall` / `transition_module` / `PolicySpec::required_attestations` / `crate::graph::build_graph_ts` 各一，另 `validate/mod.rs` 一处 `unclosed HTML tag \`String\``），上 `RUSTDOCFLAGS="-D warnings"` 门禁需先清理，记为独立待办）；第五处 `missing ⊆ unresolved` 残留注释（上轮声称修完四处、实漏此处）；`cmd_validate_state` doc 称 `ghost_refs` 条目「同时在 `still_blocked` 里」——改用全模块扫描后**不成立**（残留类不进 `still_blocked`）。
