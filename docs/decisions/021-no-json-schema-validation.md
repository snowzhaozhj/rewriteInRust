...
1. ~~**`blocked_by` 幽灵引用无检出**~~ **✅ 检出已收口**（PR #88，master `eb94959`）；**处置见 [MDR-022](022-ghost-blocked-by-repair-boundary.md)**（`state repair --clear-ghost-blocked-by`，PR #90 已于 2026-08-13 合并到 master `dac8950`）。落地方式与原修法方向一致（`validate_state` 加扫描 + 告警，不硬判损坏）。三点落地细化，均非原记账所能预见：
   ...
   - **负向实证八轮**（独立 worktree，非推断，全部报红且归因准确；①③ 针对中途存在过、后被移除的 `BlockedCheckResult.missing` 字段（见本文件「变更性质登记」段的过程记录），保留记录以说明当时的判据确有区分力）：① `missing` 恒空（退回二分）→ 2 core + 1 e2e 报红；② 摘掉告警扫描 → 2 core 报红；③ `missing` 不计入 `unresolved`（幽灵变可就绪）→ 3 core 报红，归因精确到「带幽灵引用的模块不得判为就绪」；④ 判据反转（合法引用当幽灵）→ 3 core 报红，其中「反向不误报」由独立测试钉住；⑤ 摘掉 `member_files` 归一 → 2 个 composite 测试报红；⑥ 环检测退回不归一 → 互锁测试报红；⑦ 歧义退回静默择一（按首个宿主判定）→ 坏划分测试报红；⑧ 摘掉排序与去重 → 3 个测试报红。
   ...
   - **告警建议曾把用户领进死路（主审 imp，已修）**：初版建议「重新 `graph build` + `state populate-modules`」，但 populate 对**非 pending** 模块一律拒绝重填（断点续传保护），而幽灵引用按定义就发生在 `blocked` 等非 pending 态 → 照做**确定性失败**。这是 MDR-020 finding 5 修过的同一失败模式。真正可执行的是 `state transition --module <M> --to <pre_blocked_status>`——离开 blocked 即清空 `blocked_by`、不丢进度、一步到位（实测有效）。根因是**e2e 只验告警文本含哪些 key、没验建议照做能成功**，已补 `smoke_ghost_reference_advice_actually_works` 端到端钉住「照做后问题真的消失」，并在幽灵态下反证 populate 路径确实被拒。
   - **⚠️ 上一条只修了一半——补跑四视角后三个视角各自独立复现了另一半（2026-08-08，已修）**。前述修复只覆盖 `module_blocked=true`（正 blocked）那类，而 `scan_ghost_references` 有意扫**全部模块**，告警分述两类却**共用同一段处置文本**。残留类（`module_blocked=false`）模块不在 blocked 态、`pre_blocked_status` **恒为 null**（该字段只在进入 blocked 时写入），照做必然失败——实测 `translating` 模块 `--to pending` 与 `--to translating` 均报 `E004 非法状态转换`，幽灵引用原样留着；而当时文案**明确否掉**的 `state reset` 才是这一类唯一可行的处置（实测一步成功，`blocked_by → None`、`validate` 转 `ok`）。
     - 修：告警按 `module_blocked` 分述处置动作。残留类改 `state reset --module <M>`，并**如实标注代价**——实测 `reviewing` 会回退到 `translating` 并清进度字段、`done`/`degrade_*` 需 `--force`、`pending`/`translating` 则原状态不变；旧文案一句「不丢迁移进度」通吃属过度承诺。`06`/`09`/`run.md` 三处同源文案同步。
     - **配套失效：为上一条加的守卫是空的**。`!advice.contains("populate-modules 同步")` 恒真（旧文案实为 `` `state populate-modules` 同步 ``，反引号在中间故不含该子串），且「照做」代码用**硬编码** `--to pending`、与文案无绑定。审查变异实证：把死路建议换措辞塞回、甚至改成确定性失败的 `--to done`，测试**照样 PASS**。修：新增 `parse_advice_command` 从 advice 里按反引号奇偶位**解析**命令再执行（不用内容启发式——`contains("state")` 会被散文里的同名词误触发），文案给错即报红。
     - **教训（本条最值得留存）**：**测试的场景定义域必须覆盖被测行为的定义域**。行为分述 N 类而 e2e 只覆盖 1 类，其余类的同型缺陷就原样留下；共用的那段（处置建议、错误提示）尤其危险，因为它**看起来**已被分述覆盖。另：审查修复本身是未审代码，超豁免阈值须再过一轮——上一轮 STATUS 记「四视角闭环」时，两个 imp 修复（合计 174 行，含删 pub 字段与重写编排步骤）自身从未过审查。
   - **`--auto-unblock` 遇环整体拒绝，而分流表曾把它与 ready 列为独立行（主审 + codex 共同必修，已修）**：`ready_to_unblock` 与 `cycles` 可同时非空，编排器若按 ready 行选 `--auto-unblock` 会拿到 `E012` + 退出码 1 且**一个模块都不解除**。实测（`READY` 可解除 + `C1↔C2` 互锁）：`ready_to_unblock=["READY"]`、`cycles` 非空 → `--auto-unblock` 返 E012，`READY` 仍 `blocked`。修：`run.md` 分流表改为**带优先级五行**（① cycles 先判并明示此时不可调 auto-unblock → ② ready → ③ ghost_refs → ④ `warnings` 类数据完整性问题**不可当正常等待跳过**（跨组 `member_files` 破坏只经 warning 报出，却会让模块落在 `still_blocked` 里）→ ⑤ 其余 still_blocked），并补 e2e 钉住该前提行为。
   - **补跑四视角的负向实证三轮**（独立 worktree，全部报红且归因准确）：① 残留类处置退回共用 transition 建议 → `smoke_residual_ghost_reference_advice_actually_works` 报红，报错文本正是原始缺陷形态；② blocked 类建议改成 `--to done`（**审查已证旧断言挡不住这个变异**）→ `smoke_ghost_reference_advice_actually_works` 报红；③ 摘掉 `--auto-unblock` 的环守卫 → `smoke_auto_unblock_refuses_entirely_when_cycles_present` 报红，输出显示 `READY` 被真的解除（`unblocked:["READY"]`）。
   - **同批 nit 已修**：三处 rustdoc 内链指向已改名的 `canonicalize_blocked_ref`（实际 `resolve_blocked_ref`；`cargo doc` 报 unresolved link，而 `just ci` **不跑 `cargo doc`** 故 CI 全绿掩盖了它——**该轮的修法本身是无效的**，见下方第三轮记录——改名只是把 `unresolved link` 换成了 `links to private item`，`cargo doc` 警告数 13 → 13 一条没少）；第五处 `missing ⊆ unresolved` 残留注释（上轮声称修完四处、实漏此处）；`cmd_validate_state` doc 称 `ghost_refs` 条目「同时在 `still_blocked` 里」——改用全模块扫描后**不成立**（残留类不进 `still_blocked`）。
   - **⚠️ 第三轮四视角（2026-08-11）：上两轮的修法**方向**就是错的，已按根因重做**。审查对象是第二轮的修复产物 `94b53aa`（249 行，超 CLAUDE.md 全部四项豁免阈值）。去重后 8 类 important，其中 4 类由 4/4 视角独立复现。**⛔ 本段描述的「处方层」（`GhostRemedy`/`remedy`/状态网格）已于第四轮整体移出 #88，见文末「第四轮 + 拆 PR」——本段保留作历史，其中关于 `reset_force_reason`（唯一真值源）与文档↔代码集合守卫的部分仍有效并保留在 #88。**
     - **根因：`module_blocked` 一个 bool 扛了四个正交谓词**——该状态能否进 blocked（决定要不要处置）、reset 是否需 `--force`、项目是否 `graduate`（决定 reset 可不可用）、blocked 有无 `pre_blocked_status` 锚点（决定 transition 目标）。于是残留支那一条命令要通吃 11 个 status。这是本 PR 内**第二次**同型错误（第一次是 `data.retryable` 兼任「错误归属」判定，被 codex 双向反例推翻），值得单独记住：**发现一个 bool 要分述两类以上后果时，先问它是不是在替多个谓词说话。**
     - 四类实测缺陷（每条编排器独立复核后才动手）：⒜ **`--force` 清单漏 `paused`**（`reset_force_reason` 实为 `Done|Blocked|Paused|degrade_*`），paused 残留模块照文案做即 `E012`，而文案刚说它不需要 `--force`；且**不只手工编辑可达**——`state recover --policy skip` 就会把模块置为 `paused` 而保留 `blocked_by`。讽刺的是 `06:123` 本来就写着正确口径，06 自己两行互相矛盾。⒝ **「transition 一律报非法转换」是假的**：`blocked_by = None` 只在 `from == Blocked` 分支执行，故 `reviewing → translating` 等合法边**返回成功而引用原样留着**——比 `E004` 危险，`status:ok` 会被读成已处置。⒞ **`done`/`degrade_*` 永不可能再进 blocked**（`Done => false`、`degrade_* => 仅 Translating`），告警对它们断言的「一旦被标 blocked 即永久阻塞」是**不可达前提**，却据此建议 reset——照做会把已完成模块打回 `translating`、清空进度、产出「删 .rs + 重译」指令，而 `validate` 转 `ok` 让破坏看起来像成功。⒟ **`graduate` 项目态下 reset 一律被拒且 `--force` 不可绕**，`done` 模块亦不可 transition —— CLI 层一条可行命令都没有，告警却永远重复它。
     - **修法（根治，非再补一处文案）**：
       - 抽 `pub fn reset_force_reason(ModuleStatus) -> Option<&'static str>` 为「哪些状态 reset 需 `--force`」的**唯一真值源**，`reset_module` 守卫与告警文案共用；变体逐个列出而非 `s if s.is_degraded()` guard，使新增状态**编译失败**而不是默默落进 `_ => None`。
       - `GhostReference.module_blocked: bool` → `status: ModuleStatus` + `remedy: GhostRemedy`。`remedy` 由三个**代码级**判据推导（`can_transition_to(Blocked)` / `reset_force_reason` / 项目态），四类：`leave_blocked{to, anchored}` / `reset{force}` / `inert` / `reopen_migration`，且**已替换好实际值**——编排器不再回读 state 二次推断（旧版占位符在锚点为 `null` 时会被替换出 `--to null`，实测 `E012`，编排器停在没有下一步的位置）。
       - 新增守卫 `reset_force_states_are_consistent_across_docs`：从 `ModuleStatus::iter()` 过 `reset_force_reason` 取真值域，与 `06`/`09`/`run.md` 三处声明句做**集合相等**比对（不是包含——只查缺失挡不住多写）。文档侧的 token 用 `ModuleStatus::from_str` 过滤，故 `--force`/`machine.rs` 这类非状态 token 自然落选，无需维护排除名单。报错文案明确写「修法是改文档跟上代码，不是改断言期望值」（守卫报错不得诱导关掉守卫）。
       - e2e 从「解析人读文案再跑硬编码 argv」改为**按机读 `remedy` 构造 argv 真跑**，并参数化成**状态网格**（blocked+锚点 / blocked+无锚点 / translating / reviewing / paused / done / degrade_skip）。上一轮那个 `parse_advice_command` 被 4/4 视角各自变异穿透（`--keep-progress` / `--ghost-purge` / `--bogus-flag` 全部 PASS）——因为它解析完之后跑的仍是手写 argv，二者毫无绑定；而按反引号奇偶位解析本身也不稳（module key 经 `serde_json` 转义但**反引号不在转义集里**，key 含一个反引号即整体错位，测试报成「告警没给命令」的假归因）。
       - 新增 `smoke_transition_on_residual_ghost_succeeds_without_clearing_blocked_by` 钉住 ⒝：命令 `rc=0` 但 `blocked_by` 仍在、`validate` 仍 `warning`。
       - `run.md` ④ 行订正：跨组歧义扫描覆盖全部带 `blocked_by` 的模块，而 `still_blocked` 只收 `status=blocked` 者，故这类模块**可能完全不出现在任何机读字段里**，warning 正文是唯一线索（旧文写「会落在 `still_blocked` 里」，3/4 视角复现为假）。
       - `--auto-unblock` 遇环的错误文案补「**本次一个模块都没有解除**」——旧文「无法自动解除」可被读成「（环上那些）无法解除」。该路径把已算出的 `data`（含 `ghost_refs`）与 warnings 整体丢弃，属 pre-existing（PR #23），修法方向（仿 `cmd_validate_rules` 经 `ErrorData.details` flatten）记入下方待办 6。
     - **rustdoc 内链：上一轮声称修好，实际一条没少**。`resolve_blocked_ref` 是**私有** fn，改名只把 `unresolved link` 换成 `links to private item`，公开文档里依旧不是可点链接、`-D warnings` 下依旧红。且上一轮记的「仓内仍有 5 处 pre-existing 断链」漏了 8 条同样会被 `-D warnings` 打死的 `links to private item`（`build_graph_inner` / `ToolProbe` / `warn_if_target_is_workspace_member` / `classify_evidence` / `check_version_compat`，加本 PR 自造的 3 条）——按它自己声明的用途（门禁前置清理清单）少算 8 项。本轮改用纯反引号，实测 `cargo doc --no-deps -p rustmigrate-core` 警告 **13 → 10**（余 10 条全为 pre-existing：4 条 unresolved link + 5 条 links to private item + 1 条 unclosed HTML tag）。**获取方式**：`cargo doc --no-deps -p rustmigrate-core 2>&1 | rg 'generated \d+ warning'`（读 rustdoc 自报计数「generated N warnings」；**不要用** `grep -c '^warning'`——它会把末尾那条 summary 行也算进去、多计 1，实测得 11 而非 10，第四轮由此纠正）。`just ci` 不含 `cargo doc`，故这些不会被 CI 抓到。
     - **负向实证七轮**（独立 worktree，全部报红且归因精确）：① 从 `reset_force_reason` 摘掉 `paused` → core 穷举测试 + `machine.rs` 既有守卫测试 + e2e 网格 **三层同时红**（证真值源确实被三个消费方共用）；② 摘掉 `can_transition_to(Blocked)` 判据 → 穷举测试红且归因到 `status=done`；③ 告警文案把「编排器不得自动补」改成「可自行补上」→ 网格红（证文案与 remedy 的同口径断言有牙）；④ `ghost_remedy_argv` 漏掉 `--force` → 网格红，报错里直接是 `E012`；⑤ 文档漏写 `paused`（**复现原始缺陷**）→ 守卫红并列出两侧集合差异；⑥ 文档多写 `translating` → 守卫红（证集合相等而非包含）；⑦ 文档锚点句被润色 → 守卫红并报「找不到锚点」而非静默跳过。
     - **设计契约视角在 rustdoc 那条上判错**（它只枚举了非 private-item 的 5 条就认定 MDR 计数准确），由编排器独立复跑 `cargo doc` 定论。记此以说明：**多视角互相矛盾时须自己跑一遍**，不能按视角数量投票。
...
## 第四轮四视角 + 拆 PR：处方层整体移出（2026-08-12）

**范围（按用户设界）**：只跑专项三子视角（测试覆盖 / 静默失败 / 注释准确）+ 异构交叉 codex；**本轮结束即收口，不起第五轮**，剩余问题记账于此。审查对象为第三轮产物 `798a026`。

**决定性发现（编排器亲自复现）**：对一个持有残留幽灵引用、且预置了完整进度字段的 `reviewing` 模块，`state transition --to blocked` 再 `--to reviewing`（**两步都不带 `--force`**）会清空 `blocked_by`、而 `substatus=awaiting_final_review`/`test_pass_rate=97%`/`coverage=85` **全部保留**，`validate state` 随即转 `ok`，代价仅 2 条审计记录。这条**零代价两步往返**证伪了第三轮处方层作为事实 ship 的三条断言：①「transition 修不了这一类」为假；② `reset` 是可用处置里**最贵**的那个（它清 6 个进度字段 + 发「删 .rs 重译」指令）；③「`graduate` 下 CLI 层无可行命令」为假（transition 无 graduate 守卫）。该往返本身依赖 `transition_inner` 的宽松（见待办 3），把它当「选项」列出即等于文档化一条可能是 bug 的路径。

**其余确认项（各有实测复现）**：`degrade_*` 判 `inert` 错——用「一步可达」而语义需「可达性」，`degrade_* --force→translating→blocked` 两步可达且恢复边不清 `blocked_by`；`leave_blocked` 原样透传非法锚点（`pre_blocked_status=done` → `--to done` → `E004`）；`graduate`+`blocked` 的 `leave_blocked` 会静默造出 MDR-015 禁止的「项目终态 + 非终态模块」而 `validate` 报 `ok`；`reset` 清**整个** `blocked_by` 数组会连带擦掉同模块的 `Ambiguous` 引用（`member_files` 跨组破坏的唯一检出通道，**✅ 该「唯一」表述已被 [MDR-023](023-cross-group-host-resolution.md) 决策 3 取代**——跨组告警改扫全量划分后不再依赖这条引用）；网格 `inert` 格断言恒真、穷举测试不真穷举、`GhostRemedy::command` 零生产调用点且已与 e2e argv 漂移；守卫 `sources` 只 `find` 首处锚点；本待办列表 `6.`/`5.` 源序颠倒致渲染错位；`grep -c '^warning'` 多计 summary 行。

**决策：拆 PR（检出留 #88，处方另开）**。规律是三、四轮下来**检出层稳**（`scan_ghost_references`/归一/环检测/歧义处理无人再提），**处方层每轮在同一形状上出错**——把「不可能」写进用户可见文案而依据只是「看出边」。既已知为假就不能合并，且按设界不起第五轮，故：
...
- **移出 #88（后续 PR）**：`GhostRemedy` enum / `ghost_remedy` 分类器 / `GhostReference.remedy` 字段 / `GhostRemedy::command` / 5 桶处方文案 / `test_ghost_remedy_*` 与 e2e 状态网格 / `ghost_remedy_argv` / run.md·06·09 的四类处置表。后续 PR 应设计**非破坏性入口**（拟 `state repair --clear-ghost-blocked-by`：只清幽灵 `blocked_by`、不动进度字段、不触发删产物），届时才谈「处置」。
  - **✅ 已落地（PR #90 于 2026-08-13 合并，master `dac8950`；见 [MDR-022](022-ghost-blocked-by-repair-boundary.md)）**。命令名与语义如拟，但**处方层没有被重建**：关键差别是 repair 对全部 11 个 status **一视同仁**（只删无处归属的条目），因此不需要任何按状态分叉的判断——前一轮的全部错误断言都产生于「为不同状态选不同命令」这个需求，需求消失，断言无处可写。上面列的那些符号一个都没回来。
