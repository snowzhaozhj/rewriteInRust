...
1. ~~**`blocked_by` 幽灵引用无检出**~~ **✅ 已收口**（后续 PR）——落地方式与原修法方向一致（`validate_state` 加扫描 + 告警，不硬判损坏）。三点落地细化，均非原记账所能预见：
   ...
   - **⚠️ 第三轮四视角（2026-08-11）：上两轮的修法**方向**就是错的，已按根因重做**。审查对象是第二轮的修复产物 `94b53aa`（249 行，超 CLAUDE.md 全部四项豁免阈值）。去重后 8 类 important，其中 4 类由 4/4 视角独立复现。
     ...
     - **rustdoc 内链：上一轮声称修好，实际一条没少**。`resolve_blocked_ref` 是**私有** fn，改名只把 `unresolved link` 换成 `links to private item`，公开文档里依旧不是可点链接、`-D warnings` 下依旧红。且上一轮记的「仓内仍有 5 处 pre-existing 断链」漏了 8 条同样会被 `-D warnings` 打死的 `links to private item`（`build_graph_inner` / `ToolProbe` / `warn_if_target_is_workspace_member` / `classify_evidence` / `check_version_compat`，加本 PR 自造的 3 条）——按它自己声明的用途（门禁前置清理清单）少算 8 项。本轮改用纯反引号，实测 `cargo doc --no-deps -p rustmigrate-core` 警告 **13 → 10**（余 10 条全为 pre-existing：4 条 unresolved link + 5 条 links to private item + 1 条 unclosed HTML tag）。**获取方式**：`cargo doc --no-deps -p rustmigrate-core 2>&1 | grep -c '^warning'`（`just ci` 不含 `cargo doc`，故这些不会被 CI 抓到）。
