...
Work Unit（一个 Claude Code 会话）:
  ...
  Step 5: Phase B — 编译修正 + 惯用化优化
    ...
    - **loom/shuttle 插桩**（仅 async/并发模块）：translator Phase B 完成时须为每个并发原语添加 cfg-gated 导入（如 `#[cfg(loom)] use loom::sync::Mutex; #[cfg(not(loom))] use tokio::sync::Mutex;`），使 L7 测试能激活 loom 状态空间探索
