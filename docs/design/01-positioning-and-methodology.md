...
- 工具层的门禁通过**独立脚本**执行，AI 无法说服自己跳过（借鉴 DAE 设计理念）
...
v0.9 将翻译内循环拆为两个显式阶段，解决"翻译和优化混在一起导致错误难以定位"的问题：

```
Phase A — 忠实翻译
  - 优先保持与源码的 1:1 对应（便于 diff 对照审查）
  - Private 方法默认翻译（不省略），保持结构完整性
  - 标记系统：TODO(port) / PERF(port) / PORT NOTE
  - F1 反馈：编译验证

对抗性审查（verifier SubAgent 介入）
  - 逐维度比对源码与翻译结果
  - 产出物：审查报告（差异列表 + 修正建议）
...
这一设计借鉴了 Bun 迁移实践中验证有效的分阶段方法——先保证语义正确，再追求惯用性。

**参考案例**：Bun 的 PORTING.md 实践被广泛引用为迁移规则的成功范例。

> **待验证说明**：Bun 团队在迁移博客中提及了 PORTING.md 的使用，但在其公开 GitHub 仓库中未找到独立的 PORTING.md 文件。该引用的具体内容（如"576 行"的描述）来自社区二手信息，需在 M0 阶段交叉验证其准确性。即使此引用不可验证，"以规则文件约束翻译行为"的方法论本身已在多个开源项目中得到独立验证。
