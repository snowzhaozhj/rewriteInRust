...
> **「3 命令」是入口数，不是「只跑 3 步」**：每个命令内部自动串联多个子步骤——`/migrate analyze` 自动执行 初始化 -> 规则生成 -> 测试搭建 共 7 步（3 次 SubAgent 调用）；`/migrate run` 自动执行 Phase A 忠实翻译 -> 对抗性审查 -> Phase B 优化 -> F2 验证（共 2 次 SubAgent 调用：translator 翻译/优化 + verifier 审查/验证）；`/migrate review` 运行完整验证管线。设计价值在于**把多步编排自动化、用户少记命令**，而非「3 条命令就够」。各子步骤间有文件存在性检查点与最多 2 次重试，单步偏差不必整体重来（编排可靠性的 M0 验证与 Plan B 降级见 [06 § 10.5](./06-plugin-structure.md#105-编排调度路径)，逐步骨架见 [09 附录 B](./09-appendix-schemas.md#附录-bmvp-skill-的-skillmd-骨架)）。**规模预期与适用范围**：本工具的「MVP」指路线图 M1，是 **50-70 人天的开发实施量（2 人约 6-9 周）**，并非「3 条命令、开箱即用」——「3 命令」是用户入口数，内部含 11 个 CLI 子命令 + 4 个 SubAgent（组件清单见 [01 § 1.3](./01-positioning-and-methodology.md#13-我们做什么)）。它是验证迁移质量的工作流、需团队实施落地（工作量权威见 [08 § M1](./08-roadmap-and-reference.md#m1-mvp6-8-周)）：**<2K 行小项目直接手动迁移更划算，性价比在 5K+ 行项目才显著**。**前置提醒**：M0 假设验证（5-10 人天）是 M1 估算的前置，若 Spike 1 或 4 失败会改变 M1 范围（强制 Plan B3，工作量抬至 60-75 人天），建议小团队先完成 M0 再承诺 M1 时间。
