...
**Step 3.3: proptest 回归基准初始化**：verifier 从源项目测试 suite 提取/生成当前模块的初始 proptest case，写入 `test-fixtures/proptest-regressions/{module}.txt` 作为 Phase B 对标基准（对应 [03 § 7.1](./03-execution-model.md#71-测试分层l0-l7) proptest 回归集管理策略第 1 点）。**检查点**：回归文件存在且含 >= 1 条 case。
...
- CLIPPY_CONF_DIR=$(project_root)/.rust-migration cargo clippy -- -D warnings — lint 检查（加载迁移专用 clippy.toml）
...
执行 `rustmigrate state transition --module <M> --to reviewing`（测试通过后进入最终签批，使断点续传可路由到 Step 5）。
