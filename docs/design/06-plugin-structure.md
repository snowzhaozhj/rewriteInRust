...
| `rustmigrate state repair` | 删除无处归属的 `blocked_by` 引用（幽灵引用）——**非破坏性**数据修复（M4，[MDR-022](../decisions/022-ghost-blocked-by-repair-boundary.md)）。修复类型 flag `--clear-ghost-blocked-by` **必填**（不存在裸 `repair`）；`--module` 可选，省略即清全部模块（一次 state↔graph 不同步常波及多个模块）。**只删幽灵条目**：`Resolved`（合法未终态依赖）与 `Ambiguous`（宿主歧义——`member_files` 跨组互斥破坏的**唯一检出通道**）原样保留，不是清空整个 `blocked_by` 数组。**不改状态、不清进度字段、不发产物清理指令**（与 `state reset` 正相反）；恢复不在本命令职责内——清完后仍 `blocked` 且已无依赖的模块由既有 `validate state --check-blocked --auto-unblock` 按 `pre_blocked_status` 恢复。输出 `{cleared:[{module,missing}], affected:[{module,status,blocked_by_remaining,pre_blocked_status}], was_noop, next, advice}`。**幂等**：无幽灵可清即空操作（`was_noop`，不落盘、不追加审计）。项目 `graduate` 态**放行**（repair 不改状态，制造不出 MDR-015 禁止的「项目终态 + 非终态模块」矛盾；拒绝只会让这类损坏永远没有修复入口）。清完后仍 `blocked`、已无剩余依赖、而恢复锚点用不了（`pre_blocked_status` 不在 `blocked` 合法出边内，或锚点缺失致 `--auto-unblock` 以 `pending` 兜底）时**告警**，锚点本身不动（改锚点会丢失恢复目标，属破坏性）。**前提**：本命令假定被引 key 不该存在；若它本应是登记模块（analyze 漏登记），删引用会让引用方**提前**解除阻塞，那种情况应重跑 analyze 重建 state |
