...
| `rustmigrate state record-metrics` | 记录 verifier 产出的模块质量度量（M4-QUAL-05）：`--test-pass-rate` 原样写入 `ModuleState.test_pass_rate`（支持 `85%` / `0.85` / `85` / `85/100`），`--known-differences` 写已知行为差异数；两者至少提供一项、支持部分覆盖更新。独立于状态转换矩阵，供 `stats quality` 计算 `behavior_coverage` / `final_score` 消费 |
...
| `rustmigrate stats quality` | 迁移质量度量（per-module `final_score` + project-wide `degrade_rate` / `behavior_coverage`），对齐 03 § 7.5 评分卡（M4-QUAL-01）。`--source` / `--rust`（省略则取配置）经 tokei `count_loc` 计算项目级 `rust/source` LOC 比，并作为明确标注的近似值统一应用到各模块；不走 `stats compare`，避免 Go/C 尚未实现控制流嵌套分析时连 LOC 比也丢失（M4-QUAL-05） |
