...
| `rustmigrate state reset` | 幂等回退失败/中途模块到干净重译入口（M4-ROB-01a）：status→`translating`、清全部进度字段（substatus/phase_a_version/audit/通过率/coverage/known_differences/blocked 锚点），保留 `attempts`（追加 reset 审计）与结构冻结字段（tier/member_files/composite_kind/decomposition_*/danger）；已在干净入口时幂等空操作；`done`/`blocked`/`degrade_*` 须 `--force`。输出 `cleanup.member_files` 源作用域驱动编排器删部分 `.rs` 产物（CLI 不猜路径删文件，见 [MDR-015](../decisions/015-reset-idempotent-retry-boundary.md)） |
