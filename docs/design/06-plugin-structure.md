...
**已实现命令 — 31 个**（含 M1 MVP 起陆续落地的 M2/M3/M4 命令；本表是 CLI 命令列表的唯一权威，新增命令须同步本表与 `plugin/skills/migrate/SKILL.md` 的命令清单——**两侧各自对 CLI 有 CI 守卫**（`cli/crates/cli/tests/design_command_table.rs`），两侧一致性由「都对齐 CLI」推出，本表与 SKILL.md 之间无直接比对边；命令**说明文本**的分歧不在守卫范围内）：
...
| `rustmigrate validate state` | 校验 `migration-state.json` 的合法性：**手写判据**，无 JSON Schema（见 [MDR-021](../decisions/021-no-json-schema-validation.md)）——schema 版本兼容 + `state_history` 链完整性（首条为 `init`、末条与当前状态一致、`exited_at` 时间链、相邻转换合法）+ 各状态前置条件 + 非法枚举值告警 + **`member_files` 跨组互斥不变量**（扫全量划分，报出宿主不唯一的文件及其全部宿主；两种破坏形态一视同仁——被 ≥2 个组列为成员，或既登记为独立模块又被别的组列为成员，见 [MDR-023](../decisions/023-cross-group-host-resolution.md)）。字段类型 / required / 枚举值域由 serde 类型化反序列化在**加载期**拒绝（返 `E010`），到不了本命令 |
...
| `rustmigrate state record-metrics` | 记录 verifier 产出的模块质量度量（M4-QUAL-05）：`--test-pass-rate` 支持 `85%` / `0.85` / `85` / `85/100`，按统一解析规则校验归一值落在 `[0,1]` 后写入 `ModuleState.test_pass_rate`（非法输入报错且不落盘）；`--known-differences` 写已知行为差异数；`--coverage <0-100>` 写测试覆盖率（签批门的 `coverage_below_threshold` 红线读它）；`--phase-a-audit-passed <true|false>` 写 Phase A 结构门结论（`headless_default` 策略与 `phase_a_audit_failed` 红线读它）；四者至少提供一项、支持部分覆盖更新。响应返回归一后的组代表 key 与写入后的实际字段值。仅运行态（translating/compile_fixing/testing/reviewing）可写，项目 graduate 与 pending/终态/暂停/阻塞/降级态拒绝；成功写入追加 `metrics:` 审计记录。独立于状态转换矩阵，供 `stats quality` 计算 `behavior_coverage` / `final_score` 消费 |
| `rustmigrate state reset` | 幂等回退失败/中途模块到干净重译入口（M4-ROB-01a）：status→`translating`、清全部进度字段（substatus/phase_a_version/audit/通过率/coverage/known_differences/blocked 锚点），保留 `attempts`（追加 reset 审计）与结构冻结字段（tier/member_files/composite_kind/decomposition_*/danger/danger_provenance）；已在干净入口时幂等空操作（`was_noop`，`cleanup.skip=true`）；`done`/`blocked`/`paused`/`degrade_*` 须 `--force`，项目 `graduate` 态一律拒绝。非 noop 输出 `cleanup.member_files` 源作用域驱动编排器删部分 `.rs` 产物（CLI 不猜路径删文件，见 [MDR-015](../decisions/015-reset-idempotent-retry-boundary.md)） |
...
| `rustmigrate state resume` | 额度耗尽/中断后的**续跑断点计划**（M4-ROB-01c）——纯查询、无副作用、不加载 graph。读已 checkpoint 的 `migration-state.json`，按状态归桶：运行态（translating/compile_fixing/testing/reviewing）→ `interrupted`（各带 `recover_command`，编排器逐个 `state recover --policy retry` 幂等重入）；**`reviewing` + substatus `awaiting_final_review` → `awaiting_approval`**（已停在译后签批门等人签批，MDR-019；**不进 `interrupted`**——`recover retry` 会清掉刚落的通过率/覆盖率证据，等于让签批白等）；`paused` → `awaiting_decision`（待人类/降级决策，不复活）；`pending` → `next`（用 `state deps <M>` 判就绪）；`blocked` → `blocked`；终态（done/degrade_*）**不重跑**，仅计入 `progress`（done/degraded/in_progress/pending/blocked/awaiting_decision/awaiting_approval/total）。额度检测归编排器/harness（CLI 观测不到 token 预算），实际重入复用 `state recover`（见 [MDR-017](../decisions/017-quota-pause-resume-boundary.md)） |
| `rustmigrate state repair` | 删除无处归属的 `blocked_by` 引用（幽灵引用）——**非破坏性**数据修复（M4，[MDR-022](../decisions/022-ghost-blocked-by-repair-boundary.md)）。修复类型 flag `--clear-ghost-blocked-by` **必填**（不存在裸 `repair`）；`--module` 可选，省略即清全部模块（一次 state↔graph 不同步常波及多个模块）。**只删幽灵条目**：`Resolved`（能归一到宿主模块的引用，与宿主是否终态无关）与 `Ambiguous`（宿主歧义——实体存在、坏的是 `member_files` 划分，处置是修划分；跨组破坏本身由 `validate state` 的**全量划分扫描**独立检出，不依赖恰好有引用指向它，见 [MDR-023](../decisions/023-cross-group-host-resolution.md)）原样保留，不是清空整个 `blocked_by` 数组。**不改状态、不清进度字段、不发产物清理指令**（与 `state reset` 正相反）；恢复不在本命令职责内——清完后剩余依赖全部终态（含清空）的模块由既有 `validate state --check-blocked --auto-unblock` 按 `pre_blocked_status` 恢复（锚点缺失时以 `pending` 兜底）。输出 `{scope, cleared:[{module,missing}], affected:[{module,status,blocked_by_remaining,pre_blocked_status}], was_noop, guidance}`——**顶层字段集在 noop / 非 noop 两条路径上恒定**，路径差异收进恒存在的 `guidance`（noop → `{skip:true, reason}`；非 noop → `{skip:false, next, advice}`），同 `reset` 的 `cleanup` / `recover` 的 `recovery`；`scope` 是归一后的收窄目标（`--module` 省略时为 `null`）。非 noop 时每个受影响模块追加一条 `attempts` 审计（`repair:cleared-ghost-blocked-by(<实际删除的元素数>)`）。**幂等**：无幽灵可清即空操作（`was_noop`，不追加审计；亦不落盘——除本次系从 `.backup` 自愈加载，那时仍写一次以修复主文件）。项目 `graduate` 态**放行**（repair 不改状态，制造不出 MDR-015 禁止的「项目终态 + 非终态模块」矛盾；拒绝只会让这类损坏永远没有修复入口）。清完后经 `check_blocked_modules` **判为就绪**（剩余依赖全部终态，含清空）、而恢复锚点用不了（`pre_blocked_status` 不在 `blocked` 合法出边内，或锚点缺失致 `--auto-unblock` 以 `pending` 兜底）时**告警**——就绪判定复用该函数、不用「剩余为空」当代理（已终态的剩余依赖不计入 `unresolved`，两者不等价），锚点本身不动（改锚点会丢失恢复目标，属破坏性）。**前提**：本命令假定被引 key 不该存在；若它本应是登记模块（analyze 漏登记），删引用会让引用方**提前**解除阻塞，那种情况应重跑 analyze 重建 state |
| `rustmigrate state batch-transition-done` | 两层 done 的**第二层门**（MDR-003 约束6 + ORCH-01）：把整组 `agent_done` 模块经**预签策略**批量升最终 `done`。并行翻译中 agent 在 worktree 内自检过只标 `agent_done`（substatus，非终态）；编排器合并本层全部分支并跑整组 `cargo check`/`test` **全过**后调用本命令（`--module` 可重复，入口去重）。整组 check 是唯一 done **技术**真门（orphan rule/E0119/feature/宏/命名空间等跨并发兄弟冲突只有整组编译才暴露），见 § M2 写隔离约束4。**译后签批门（[MDR-019](../decisions/019-post-translation-review-gate.md)）**：本命令走 `reviewing → done` 边，而签批门坐落在**这条边本身**——整组 check 通过**不构成签批凭据**，故须携带 `--by-policy <id> --attest <k>…`（用户在 `[review_gate].auto_approve_policies` 预签的窄策略 + 编排器逐项声明的产物级自查）；**未携带则一个都不放行**（全部 `skipped.code=approval_required`，并行译好的模块须逐个 `state approve` 人签批，即 MDR-018「并行翻译 + 串行审批」）。substatus 为 `awaiting_final_review`（已判定需人签批）的模块**一律拒绝**（`skipped.code=awaiting_final_review`），任何策略都不放行。逐模块独立处理（一个失败不影响其他），输出 `{requested, requested_approval, succeeded, skipped:[{module,code,detail}], duplicates}`（`requested_approval` 回显**请求的**凭据 `policy:<id>`，非结果——全 skip 时 `succeeded` 为空即可判断没放行任何模块）——`skipped`/`duplicates` 非空时 status 降级 warning；`skipped.code` ∈ `not_found`/`broken_partition`（`member_files` 跨组互斥被破坏、该 key 的宿主不唯一——处置是修划分而非补登记，与 `not_found` 分列是因为排查方向相反，见 [MDR-023](../decisions/023-cross-group-host-resolution.md)）/`approval_required`/`awaiting_final_review`/`not_agent_done`/`not_reviewing`/`unknown_policy`/`policy_not_enabled`/`mandatory_manual`/`policy_conditions_unmet`/`missing_attestations`/`unexpected_rejection`（末项为兜底：拒因文本未命中已知码，说明 CLI 侧新增了未登记的拒因） |
...
| `rustmigrate scaffold workspace` | 生成迁移目标 Rust 项目的基础骨架（委托 `cargo init --lib --vcs none`）；dev-dependencies 与 `deny.toml` 由 scaffolder SubAgent 按项目测试需求注入。同时写 `.gitignore`（`/target`，后置条件式幂等——缺有效规则则追加、保留用户既有内容）；`cargo init --vcs none` 不产 VCS 文件，缺它会让 worktree 自检产物被 `git add -A` 吞入提交污染合并（ORCH-01 PR-5 实测）。**产出物实为单 crate（`[package]` + `src/lib.rs`），不含 `[workspace]` 段**——单 crate 输出是既定设计（见下方 § M2 写隔离约束「worktree 是并行机制，与输出 crate 结构正交，M2 沿用单 crate 输出」），命令名中的 workspace 是历史沿称，保留以免破坏 `scaffolder.md` / `workflow.md` / `SKILL.md` 的既有调用；真需要多 crate 分层时另开命令而非改本命令语义。**目标落在已有 workspace 的仓库内时会牵连用户的构建配置**：cargo 可能把新 crate 纳入外层 workspace（显式追加进 `members`，或被既有 glob 如 `crates/*` 直接覆盖——后者**不改** manifest），此后该仓库 `cargo build --workspace`/`test --workspace` 会连带编译迁移产物（若该 workspace 无根 package 且未配 `default-members`，在 workspace 根执行的裸 `cargo build` 同样会），而迁移中的 crate 常处于 `unimplemented!()` 等不可编译中间态。检测判据是**状态**而非变化量——调 `cargo metadata --no-deps` 问「目标是否已在 `workspace_members` 里」（它自己解析 members/glob/`exclude`/`default-members`，是权威真值源）；幂等重跑亦按状态重新判定。**两类情形均降级 `status=warning`**：① 确认成为成员（处置建议须同时提 `exclude`——仅移 `members` 会让 cargo 报 `current package believes it's in a workspace when it's not`）；② 成员关系**无法判定**（目标在某 Cargo 项目内但 `cargo metadata` 失败）——如实报无法判定、提示手工确认，不假装已确认无事。**不报错**（成为 member 本身无破坏、用户也可能确实想要），能否接受由用户判断。判据选择的理由（含两种被实证推翻的朴素方案）与破坏性变更登记见 [MDR-020](../decisions/020-scaffold-workspace-membership-detection.md) |
...
| `BLOCKED_BY_VALIDATION_FAILED` | **语义码未落地，但校验已落地为告警**（M4：`validate state` 检出幽灵引用并降级 `status:warning`，`--check-blocked` 另在 `data.ghost_refs` 列出逐条明细 `{module, missing, status}`；不返本码、不硬判损坏——旧文件须可读） | `blocked_by` 引用了**无处归属**的 key（既非登记模块，也不是任何 composite 组的成员——成员 key 会先归一到组代表，不算幽灵） | **处置：`state repair --clear-ghost-blocked-by`**（[MDR-022](../decisions/022-ghost-blocked-by-repair-boundary.md)）。`ghost_refs` 每条为纯检出事实 `{module, missing, status}`，`status=blocked` 表示此刻就永久阻塞、其余表示残留引用；repair 对全部状态**一视同仁**地只删无处归属的条目（可归一到宿主模块的合法引用（`Resolved`，含宿主已终态的）与宿主歧义引用保留，不清整个数组），不改状态、不清进度字段——正因动作单一，它不需要任何按状态分叉的「处方」，而按状态推导处置命令的那一层曾把「不可能」写进用户可见输出、依据只是「看出边」（一步可达），已被审查全数推翻（见 [MDR-021](../decisions/021-no-json-schema-validation.md)）。清完后剩余依赖全部终态（含清空）者由 `--auto-unblock` 按 `pre_blocked_status` 恢复（锚点缺失时以 `pending` 兜底）（repair 自己不改状态）。**前提**：被引 key 确实不该存在；若它本应是登记模块（analyze 漏登记），删引用会让引用方**提前**解除阻塞，那种情况应重跑 analyze 重建 state。三条别拿来代替它的路：⑴ **transition 不是通用处置**——`blocked_by` 只在离开 blocked 的边上清空，对非 blocked 模块 transition 要么报 `E004`、要么**返回成功而引用原样留着**（后者更危险，`status:ok` 会被误读成已处置）；⑵ **不可用 `populate-modules`**——它对非 pending 模块拒绝重填；⑶ **`reset` 代价过大**——清全部进度字段并要求编排器删 `.rs` 重译，且它清的是**整个** `blocked_by` 数组，会连带擦掉宿主歧义引用（该引用本身仍应保留：实体存在、坏的是 `member_files` 划分；跨组破坏由 `validate state` 的全量划分扫描独立检出，见 [MDR-023](../decisions/023-cross-group-host-resolution.md)）。`--auto-unblock` 对仍持幽灵引用的 `status=blocked` 者**不放行**（等待依赖就绪是无效动作）。reset 需 `--force` 的状态（值域取自 `machine.rs` 的 `reset_force_reason`，勿手工维护——有 CI 守卫比对）：`blocked`、`done`、`degrade_ffi`、`degrade_manual`、`degrade_skip`、`paused` |
...
[review_gate]                        # 译后签批门（M4 MDR-019 已落地，见 09 § 合法状态转换）
auto_approve_policies = []           # 用户**预签**的自动放行策略白名单；默认空 = 每个模块译完都停 `reviewing + awaiting_final_review` 等人签批。
                                     # 可选：batch_mechanical（composite_kind=batch 全机械组）/ headless_default（通过率 100% + Phase A 结构门过 + coverage 达 [testing].coverage_threshold）。
                                     # 预签只是必要条件——强制人工红线（danger 非空 / 分类来源不可信 / 已知差异 / requires_manual_review·incomplete / 结构门未过）命中时任何策略都不放行。
