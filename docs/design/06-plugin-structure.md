...
**已实现命令 — 30 个**（含 M1 MVP 起陆续落地的 M2/M3/M4 命令；本表是 CLI 命令列表的唯一权威，新增命令须同步本表与 `plugin/skills/migrate/SKILL.md` 的命令清单——**两侧各自对 CLI 有 CI 守卫**（`cli/crates/cli/tests/design_command_table.rs`），两侧一致性由「都对齐 CLI」推出，本表与 SKILL.md 之间无直接比对边；命令**说明文本**的分歧不在守卫范围内）：
...
| `BLOCKED_BY_VALIDATION_FAILED` | **语义码未落地，但校验已落地为告警**（M4：`validate state` 检出幽灵引用并降级 `status:warning`，`--check-blocked` 另在 `data.ghost_refs` 列出逐条明细 `{module, missing, status, remedy}`；不返本码、不硬判损坏——旧文件须可读） | `blocked_by` 引用了**无处归属**的 key（既非登记模块，也不是任何 composite 组的成员——成员 key 会先归一到组代表，不算幽灵） | **逐条照 `ghost_refs[].remedy` 做**（CLI 按四个正交谓词算出、已替换好实际值，编排器不再自己推断）。四类：`leave_blocked`（模块正 blocked）→ `state transition --module <M> --to <remedy.to>`，离开 blocked 是**唯一**会清空 `blocked_by` 的边、不丢进度，`anchored=false` 表示无 `pre_blocked_status`、CLI 已代入 `pending`；`reset`（残留引用）→ `state reset --module <M>`，`remedy.force` 非空时须追加 `--force` 且**属人类确认、编排器不得自动补**；`inert`（`done`/`degrade_*`，**永不可能再进 blocked**）→ **无需处置**，引用惰性无害；`reopen_migration`（项目 `graduate`）→ CLI 层无可行命令，须先重开迁移。**transition 不是通用处置**：`blocked_by` 只在离开 blocked 的边上清空，对非 blocked 模块 transition 要么报 `E004`、要么**返回成功而引用原样留着**（后者更危险，`status:ok` 会被误读成已处置）。reset 代价：`reviewing`/`testing`/`compile_fixing` 回退到 `translating`，`pending`/`translating` 的 status 不变，但**两者都会清空** substatus/phase_a_version/phase_a_audit_passed/test_pass_rate/coverage/known_differences。有可执行 remedy 者处置后均须 `graph build` 重建图。**不可用 `populate-modules`**——它对非 pending 模块拒绝重填。`--auto-unblock` 对 `status=blocked` 者**不放行**（等待依赖就绪是无效动作）。reset 需 `--force` 的状态（值域取自 `machine.rs` 的 `reset_force_reason`，勿手工维护——有 CI 守卫比对）：`blocked`、`done`、`degrade_ffi`、`degrade_manual`、`degrade_skip`、`paused` |
