...
- **升级检查清单**：① 保留旧 Plugin 备份 → ② `rustmigrate validate config`（minor/major）→ ③ 审查 warnings → ④ 手动复查关键产出物是否仍可编译/运行 → ⑤ 确认后更新。
...
- **L1/L2 适用对象界定（R2-D3-03）**：Markdown 产出物（`{module}-intent.md`、`porting/` 规则、`{module}-review.md`）与代码/配置产出物无 JSON Schema，只做 **L1**（存在、非空、关键标题存在）；JSON 产出物（`migration-state.json`、测试结果 JSON）做 **L2 结构校验**（格式合法、关键字段非空、可选的引用一致性，如 `blocked_by` 引用的模块名须存在于 `modules` 键中）；SQLite（`source-graph.db`）做 **L2 表结构校验**（必要表存在、版本号合法）。L2 各产出物的具体检查项以「L2 校验规则速查表」（[09-appendix-schemas.md 附录 A](./09-appendix-schemas.md#附录-amigration-statejson-schema)）为权威；该表是 `rustmigrate validate state` 的设计输入，MVP 不要求 100% 自动化（语义校验 L3 见 § 10.5，现阶段为人工 sampling）
...
- **多终端并发冲突的强制隔离（R2-D5-02）**：「单一实例独占」由**全局命令锁**强制执行，而非依赖约定。每个 `/migrate` 命令的 SKILL.md 在 **Step 0「全局锁获取」**（一切操作之前）以 `flock -n .rust-migration/.migration-lock --` 非阻塞获取全局锁；若另一 `/migrate` 命令正在运行，非阻塞获取失败（非零退出码），Skill 立即向用户报错并退出：「检测到另一个 `/migrate` 命令正在运行；请等待其完成，或在确认无进行中任务后手动删除 `.rust-migration/.migration-lock`」——**不自动退避重试**（避免两个进程都通过锁后并发写 `migration-state.json`）。锁在末步完成或错误处理器中释放。陈旧锁（异常终止残留）的检测阈值由 `.rustmigrate.toml` `[orchestration].lock_timeout_secs`（默认 3600）控制。Step 0 锁协议骨架与故障恢复（异常终止后锁残留的清理）见 [09-appendix-schemas.md 附录 B](./09-appendix-schemas.md#附录-bmvp-skill-的-skillmd-骨架)。
...
**常见迁移错误码（与 SubAgent substatus 对应）**：下表仅列主路径高频码示例；**完整枚举（30-40 条，按「编译错误 / 翻译语义错误 / 验证失败 / 状态机错误 / SubAgent 错误」五类组织，每条含稳定码名、含义、触发条件、CI 推荐重试策略、诊断参考链接）以 [09-appendix-schemas.md § A.5 完整错误码表](./09-appendix-schemas.md#附录-amigration-statejson-schema) 为唯一权威**（R1-BS2-01）。
...
lock_timeout_secs = 3600             # 全局命令锁（.migration-lock）陈旧检测阈值（见 § 10.5 多终端并发隔离）
