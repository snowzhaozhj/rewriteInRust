...
| `rustmigrate state record-metrics` | 记录 verifier 产出的模块质量度量（M4-QUAL-05）：`--test-pass-rate` 支持 `85%` / `0.85` / `85` / `85/100`，按统一解析规则校验归一值落在 `[0,1]` 后写入 `ModuleState.test_pass_rate`（非法输入报错且不落盘）；`--known-differences` 写已知行为差异数；两者至少提供一项、支持部分覆盖更新。响应返回归一后的组代表 key 与写入后的实际字段值。仅运行态（translating/compile_fixing/testing/reviewing）可写，项目 graduate 与 pending/终态/暂停/阻塞/降级态拒绝；成功写入追加 `metrics:` 审计记录。独立于状态转换矩阵，供 `stats quality` 计算 `behavior_coverage` / `final_score` 消费 |
