...
Skill SKILL.md（分步指令）
  ...
  │
  ├── Step (conditional): "若 source-graph.db 含 purity_confidence=high 纯函数且其参数/返回类型均为 JSON-safe（原语、plain object、array、string、number、boolean、null；不含 Map/Set/RegExp/Symbol/Buffer/函数引用——不满足时归入 [03 § 7.6.1](./03-execution-model.md#761-库函数-ffi-对比可行性检查清单) 三选一的「部分纯化」行），执行 adapters/{language}/ffi-bridge.sh <source_root>"
