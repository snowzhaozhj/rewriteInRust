...
- **构建与一致性**：GitHub Actions 矩阵构建（`os: [ubuntu-latest, macos-latest]` × 目标三元组，arm64 用 `cross`）；`release.yml` post-build 步骤对每个二进制执行 `./binary --version` 比对 `plugin.json` version，不匹配则 fail release（与 § 10.0.2 版本一致性检查同源）。矩阵额外包含一个 MSRV 构建作业（`cargo build --locked` 在声明的 `rust-version` 上执行），确保不引入高版本 API。
...
| **translator**（Phase A/B） | `{module}-intent.md` + 规则 + 依赖接口 | Rust 源文件（写入 `rust_root/` 目录，由 `.rustmigrate.toml` 配置）、`_porting_manifest.json`（写入 `.rust-migration/context/module-learnings/{module}/`） | 模块 translating 且意图已确认（Step 1.5 人类确认门禁通过） | Rust 文件存在且通过 F1 编译；manifest 至少含一条 RULE 引用 | **L1 存在性**：Rust 文件存在且 F1 编译通过；manifest 存在且非空 |
...
>
> **该锁在 MVP 的真实目的**：对 `source-graph.db` 的 `flock` 排他锁是**防御性编程**——应对「指令跟随失败导致多个 SubAgent 意外并发」的小概率情况。正常 MVP 流程（analyzer→translator→scaffolder 严格串行，序列见 § 10.5）下此锁不会被竞争。M2 引入跨模块并行（见 § 10.5）后，该机制才成为关键防护（届时配合 04 § 5.7.3 的 WAL + busy_timeout）。
>
...
**F2 和 F3 的实现方式**：
- **F2（模块完成后验证）**：通过 `hooks/scripts/verify.sh` 独立脚本执行 `cargo nextest run --lib` + `cargo clippy -- -D warnings`；`verify.sh` 在调用 `cargo clippy` 前须设置 `CLIPPY_CONF_DIR=$(project_root)/.rust-migration`（从 `.rustmigrate.toml` 的 `migration_root` 或 `$MIGRATION_ROOT` 环境变量推导），因 `.rust-migration/` 不是 `rust_root` 祖先目录、Clippy 默认查找无法命中（见 [04 §5.2](./04-toolchain.md#52-tier-0硬性门禁)）。若 migration-state.json 标记模块为 async/concurrent，追加 `RUSTFLAGS='--cfg loom' cargo nextest run --test loom_*`（或 shuttle 等价调用）。Skill SKILL.md 中指令调用该脚本，但脚本本身是确定性的，agent 无法修改或跳过其内部逻辑。
...
| `source-graph.db`（SQLite） | 单写者；CLI `graph build` 与 analyzer 串行 | M2 备选策略（当 WAL 写锁等待 >50ms 时启用）：analyzer SubAgent 启动时 `PRAGMA query_only=ON` 强制只读；汇总写入集中到单 writer 阶段。默认策略（WAL 串行写）见 [04 § 5.7.3](./04-toolchain.md#573-持久化存储)，决策框架见 [08 §M2 图并发写策略](./08-roadmap-and-reference.md#m2-质量提升8-12-周) |
...
enforce_rule_version_consistency = true     # verifier 拒绝混用不兼容的规则版本
...
│   └── tests/                  # 验收基准数据（精度/覆盖率对比源）
│       ├── fixtures/           # 最小样本项目（供端到端跑通适配器脚本）
│       └── expected/           # 人工标注 ground-truth（type-map.json, deps.json）
├── python/
│   ├── adapter.json
│   ├── detect.sh
...
Skill 的 SKILL.md 通过 Claude Code 的 Bash tool 执行适配器目录下的 shell 脚本。`source_language` 未设置时，SKILL.md 执行 `ls adapters/*/adapter.json` 枚举可用适配器，逐一调用 `detect.sh`，取返回 0 的适配器；多个返回 0 则按 § 11.3 语言热图权重排序并报多语言场景（M2+）；此枚举为 M2 实现，MVP 要求 `source_language` 必填。调用链路如下：
...
Skill SKILL.md（分步指令）
  ...
  │
  ├── Step (conditional): "若 source-graph.db 含 purity_confidence=high 纯函数且其参数/返回类型均为 JSON-safe（原语、plain object、array、string、number、boolean、null；不含 Map/Set/RegExp/Symbol/Buffer/函数引用），执行 adapters/{language}/ffi-bridge.sh <source_root>"
  │     └── Bash tool → ffi-bridge.sh → 输出子进程桥接脚本到 test-fixtures/ffi-bridge/
  │
  ├── Step (conditional): "若纯函数参数/返回类型非 JSON-safe 但为 structured-clone 兼容（Map, Set, TypedArray），使用反转测试运行器（[03 § 7.6](./03-execution-model.md#76-行为等价性验证) 注）执行 L2 等价对比——Node.js 端原生构造实例调用 TS 旧实现，napi-rs 侧通过 JsMap/JsObject 映射传递给 Rust 新实现，结果对比输出 JUnit XML。函数含真正不可序列化类型（闭包、Symbol、带状态 RegExp）时，标记 requires_manual_review（等同三选一第三行降级）"
  │     └── 反转测试运行器 → JUnit XML 到 test-fixtures/ffi-bridge/
  │
  └── Step: "读取 adapters/{language}/porting-template.md 作为 PORTING 规则初始模板"
        └── Read tool → porting-template.md → 注入 translator SubAgent 上下文
```

脚本约定（各脚本的机读输入/输出 Schema 见 [§ 11.2.1](#1121-适配器脚本输入输出-schema)）：
...
### 11.2.1 适配器脚本输入/输出 Schema

§ 11.2 的脚本接口此前仅以自然语言描述，无机读契约，社区贡献的脚本难以校验输出格式是否匹配（R1-D7-06）。下表给出各脚本的输出契约，**所有结构化 Schema 复用 09 附录的权威定义，本节不重复全文**：

| 脚本 | 输出形式 | 契约 / 权威 Schema |
|------|---------|-------------------|
| `detect.sh` | 退出码 0（匹配）/ 1（不匹配）；可选 stderr JSON | 退出码为唯一契约；如输出诊断，stderr 用 `{status, warnings}`（见 § 10.0.1） |
| `extract-types.sh` | `intermediate/type-map.json` | 复用 [09-appendix-schemas.md § type-map.json](./09-appendix-schemas.md#附录-d关键中间产物-schema简化版)（M2 参考格式） |
| `extract-deps.sh` | 依赖 JSON → 合并入 `source-graph.db` | 字段须可映射到 source-graph 节点/边（见 [09-appendix-schemas.md § source-graph 导出格式](./09-appendix-schemas.md#附录-d关键中间产物-schema简化版)）：每条依赖映射为 `imports`/`uses_type` 边，源/目标对应节点 `id` |
| `porting-template.md` | Markdown（YAML frontmatter + 规则正文） | frontmatter 格式复用 [05-documentation-system.md § 6.1](./05-documentation-system.md#61-核心产出物总览)；对适配器的特殊约定：必含 `language_id`、`rule_version` 字段，至少一条 `## 类型映射` 标题；除必含 `## 类型映射` 外，应含所有与源语言存在惯用法差异的 MVP 通用规则类对应节（参考 [05 § 6.2.1](./05-documentation-system.md#621-跨适配器特化示例) 跨适配器特化示例） |

> **CI 校验**：`adapter.json` 由 § 11.2 的 JSON Schema 自动校验；`extract-types.sh` / `extract-deps.sh` 的输出 JSON 在适配器端到端测试（工作量表「测试」行）中按上述权威 Schema 校验，作为社区 PR 合入门禁。
...
| 适配器脚本/组件 | TS 基准工作量 | 置信度 | 多语言风险因子 | 说明 |
|----------------|--------------|--------|--------------|------|
| `detect.sh` | 0.5 人天 | 95% | 低（仅特征文件） | 项目特征文件检测 |
| `extract-types.sh` | 1-2 人天 | 60% | Mypy 成熟度 / libclang 配置 | 调用语言类型工具，输出统一格式 |
| `extract-deps.sh` | 1-2 人天 | 70% | C/C++ 无统一 dep 工具，可能需自建 | 依赖图提取，合并入 source-graph.db |
| `porting-template.md` | 1-1.5 人天 | 80% | 语言惯用法差异（C 宏 / Python 动态） | 语言专用迁移规则预置 |
| `ffi-bridge.sh`（可选） | 1-2 人天 | 65% | bindgen 版本稳定性 / PyO3 ABI | FFI 桥接配置 |
| 测试 | 1 人天 | 85% | 端到端样本可得性 | 适配器端到端验证 |

> **推广系数与成本重校触发器（R1-D7-03）**：上述基准的 TS 数据点经 M1 验证，但在 Python / C++ 上的成立度仅为表中「置信度」列所示（`extract-types.sh` / `ffi-bridge.sh` 因依赖外部工具成熟度，置信度最低）。社区贡献新适配器的预期周期为 **3-5 个工作日**（贡献流程承诺与 PR SLA——如「贡献超 7 天可拆为 M2 子任务」——见 Plugin 仓库 CONTRIBUTING.md）。**触发条件**：M2 任一 Python/C++ 适配器实际工作量超估 > 20% 时，须在进入下一语言前发起一次紧急成本复审。该拆解的真实成本回顾以 [08-roadmap-and-reference.md § 13.3 多语言适配器成本回顾](./08-roadmap-and-reference.md#m1-mvp-6-8-周)（**M1 强制交付 gate，而非 M1 后置项**）为权威数据来源。
...
| 验收项 | 默认阈值 | 校验方式 |
|--------|---------|---------|
| `extract-types.sh` 类型提取精度 | `type_precision_threshold = 0.95` | 与 `tests/expected/type-map.json` 人工标注对比 |
| `extract-deps.sh` 依赖覆盖率 | `dep_coverage_threshold = 0.90` | 与 `tests/expected/deps.json` 已知依赖清单对比 |
