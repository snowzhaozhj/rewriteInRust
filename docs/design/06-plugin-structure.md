...
| `rustmigrate state batch-transition-done` | 两层 done 的**第二层门**（MDR-003 约束6 + ORCH-01）：把整组 `agent_done` 模块批量升最终 `done`。并行翻译中 agent 在 worktree 内自检过只标 `agent_done`（substatus，非终态）；编排器合并本层全部分支并跑整组 `cargo check`/`test` **全过**后，用一条 `--module` 可重复的命令批量升 `done`。委托 `batch_transition_done`：逐模块 `reviewing → done`，substatus 非 `agent_done` 跳过、当前非 `reviewing` 被矩阵拒绝，一个失败不影响其他，均记入 `attempts`。输出 `{requested, succeeded, skipped}`——`skipped` 非空时 status 降级 warning。整组 check 是唯一 done 真门（orphan rule/E0119/feature/宏/命名空间等跨并发兄弟冲突只有整组编译才暴露），见 § M2 写隔离约束4 |
