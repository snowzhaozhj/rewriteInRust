...
| `rustmigrate state record-metrics` | 记录 verifier 产出的模块质量度量（M4-QUAL-05）：`--test-pass-rate` 支持 `85%` / `0.85` / `85` / `85/100`，按统一解析规则校验归一值落在 `[0,1]` 后写入 `ModuleState.test_pass_rate`（非法输入报错且不落盘）；`--known-differences` 写已知行为差异数；两者至少提供一项、支持部分覆盖更新。响应返回归一后的组代表 key 与写入后的实际字段值。独立于状态转换矩阵，供 `stats quality` 计算 `behavior_coverage` / `final_score` 消费 |
...
| `rustmigrate state batch-transition-done` | 两层 done 的**第二层门**（MDR-003 约束6 + ORCH-01）：把整组 `agent_done` 模块批量升最终 `done`。并行翻译中 agent 在 worktree 内自检过只标 `agent_done`（substatus，非终态）；编排器合并本层全部分支并跑整组 `cargo check`/`test` **全过**后，用一条 `--module` 可重复（入口去重）的命令批量升 `done`。委托 `batch_transition_done`：逐模块 `reviewing → done`，substatus 非 `agent_done` 跳过、当前非 `reviewing` 被矩阵拒绝、模块不存在报错跳过，一个失败不影响其他。输出 `{requested, succeeded, skipped, duplicates}`——`skipped`/`duplicates` 非空时 status 降级 warning。整组 check 是唯一 done 真门（orphan rule/E0119/feature/宏/命名空间等跨并发兄弟冲突只有整组编译才暴露），见 § M2 写隔离约束4。**⚠️ MDR-019 护栏**：本命令走 `reviewing → done` 边，而 [MDR-019](../decisions/019-post-translation-review-gate.md) 的译后签批门恰坐落在**这条边本身**（`reviewing + awaiting_final_review → 批准 → done`）。MDR-019 落地后，编排器**不得**对 substatus 为 `awaiting_final_review` / 命中强制人工清单的模块调用本命令，否则绕过人签批；本命令仅用于并行翻译「合并 + 整组 check 过 → done」的自动路径 |
...
| `rustmigrate stats quality` | 迁移质量度量（per-module `final_score` / `behavior_coverage` + project-wide `degrade_rate`），对齐 03 § 7.5 评分卡（M4-QUAL-01）。`--source` / `--rust`（省略则取配置）经 tokei `count_loc` 计算项目级 `rust/source` LOC 比，并作为明确标注的近似值统一应用到各模块；不走 `stats compare`，避免 Go/C 尚未实现控制流嵌套分析时连 LOC 比也丢失（M4-QUAL-05） |
