...
| `BLOCKED_BY_VALIDATION_FAILED` | **语义码未落地，但校验已落地为告警**（M4：`validate state` 检出幽灵引用并降级 `status:warning`，`--check-blocked` 另在 `data.ghost_refs` 列出逐条明细 `{module, missing, module_blocked}`；不返本码、不硬判损坏——旧文件须可读） | `blocked_by` 引用了**无处归属**的 key（既非登记模块，也不是任何 composite 组的成员——成员 key 会先归一到组代表，不算幽灵） | `state transition --module <M> --to <pre_blocked_status>`（离开 blocked 即清空 `blocked_by`，不丢进度）后 `graph build` 重建图。**不可用 `populate-modules`**——它对非 pending 模块拒绝重填，而幽灵引用按定义发生在 blocked 等非 pending 态；`state reset` 亦不解决（置为 `translating`，仍非 pending）。`--auto-unblock` 对 `module_blocked=true` 者**不放行**（等待依赖就绪是无效动作） |
