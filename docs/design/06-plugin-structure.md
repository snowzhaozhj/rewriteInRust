...
Skill 的 SKILL.md 通过 Claude Code 的 Bash tool 执行适配器目录下的 shell 脚本。`source_language` 未设置时，SKILL.md 执行 `ls adapters/*/adapter.json` 枚举可用适配器，逐一调用 `detect.sh`，取返回 0 的适配器；多个返回 0 则按 § 11.3 语言热图权重排序并报多语言场景（M2+）；此枚举为 M2 实现。
