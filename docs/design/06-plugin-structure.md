...
| `BLOCKED_BY_VALIDATION_FAILED` | **语义码未落地，但校验已落地为告警**（M4：`validate state` 检出幽灵引用并降级 `status:warning`，`--check-blocked` 另在 `data.ghost_refs` 列出逐条明细 `{module, missing, module_blocked}`；不返本码、不硬判损坏——旧文件须可读） | `blocked_by` 引用了**无处归属**的 key（既非登记模块，也不是任何 composite 组的成员——成员 key 会先归一到组代表，不算幽灵） | **按 `module_blocked` 分两类**：`true`（模块正 blocked）→ `state transition --module <M> --to <pre_blocked_status>`（离开 blocked 即清空 `blocked_by`，不丢进度）；`false`（残留引用，模块不在 blocked 态、`pre_blocked_status` 恒为 null）→ `state reset --module <M>`（transition 对它一律报 `E004 非法状态转换`；reset 会把 `reviewing` 等回退到 `translating` 并清进度字段，`done`/`degrade_*` 需 `--force`，`pending`/`translating` 原状态不变）。两类处置后均须 `graph build` 重建图。**不可用 `populate-modules`**——它对非 pending 模块拒绝重填。`--auto-unblock` 对 `module_blocked=true` 者**不放行**（等待依赖就绪是无效动作） |
