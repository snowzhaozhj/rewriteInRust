...
- **构建与一致性**：GitHub Actions 矩阵构建（`os: [ubuntu-latest, macos-latest]` × 目标三元组，arm64 用 `cross`）；`release.yml` post-build 步骤对每个二进制执行 `./binary --version` 比对 `plugin.json` version，不匹配则 fail release（与 § 10.0.2 版本一致性检查同源）。
- **ensure-cli.sh 容错**：二进制检测/下载失败时降级提示用户 `cargo install rustmigrate` 或手动下载链接，告知该平台为 best-effort。
...
| **scaffolder** | `source-graph.db`、模块接口信息 | `test-fixtures/golden/` 测试数据、Cargo.toml dev-deps 注入 | analyzer 已完成 | 测试基础设施可运行 | **L1 存在性**：`test-fixtures/golden/` 非空、Cargo.toml 含注入的 dev-deps（文件/配置产出物，仅做 L1） |
...
- **L1/L2/L3 适用对象界定（R2-D3-03，权威定义）**：本节与 § 10.2 接口表「Schema 校验规则」列共同构成产出物分级的权威定义（§ 10.5「产出物有效性分级」给出 L1/L2/L3 三级语义，不重复逐产出物清单）。
  ...
  - **L2（结构校验）**：JSON 产出物（`migration-state.json`、测试结果 JSON）做格式合法、关键字段非空校验；SQLite（`source-graph.db`）做表结构校验（必要表存在、版本号合法）。`{module}-intent.md` 在 M1 例外升 L2（6 字段 JSON Schema 见 [09 附录 E](./09-appendix-schemas.md#附录-e意图摘要-moduleintentmd-内容规范)）。
...
>
> **该锁在 MVP 的真实目的**：对 `source-graph.db` 的 `flock` 排他锁是**防御性编程**——应对「指令跟随失败导致多个 SubAgent 意外并发」的小概率情况。正常 MVP 流程（analyzer→translator→scaffolder 严格串行，序列见 § 10.5）下此锁不会被竞争。M2 引入有限并行（analyzer + scaffolder）后，该机制才成为关键防护（届时配合 04 § 5.7.3 的 WAL + busy_timeout）。
>
...
> **检查点失败处理**：每步检查点失败（L1 文件缺失或 L2 校验不通过）时，自动重试最多 2 次（间隔 5s/15s）；2 次后报错并输出 SubAgent 调用日志（记录于 `migration-state.json` 的 `subagent_calls` 数组，每条 `{step_index, subagent_name, started_at, ended_at, status, error_message}`，Schema 见 [09-appendix-schemas.md](./09-appendix-schemas.md#附录-bmvp-skill-的-skillmd-骨架)）供排查。**各产出物适用 L1 还是 L2** 以 § 10.2 接口表「Schema 校验规则」列与 § 10.2.1「L1/L2/L3 适用对象界定」为准（本节只定义三级语义，不重复逐产出物清单）。各步的具体失败行为见 09-appendix-schemas.md 附录 B 的「Checkpoint 失败处理」表。
>
...
- **多终端并发冲突的强制隔离（R2-D5-02）**：「单一实例独占」由**全局命令锁**强制执行，而非依赖约定。每个 `/migrate` 命令的 SKILL.md 在 **Step 0「全局锁获取」**（一切操作之前）以 `flock -n .rust-migration/.migration-lock --` 非阻塞获取全局锁；若另一 `/migrate` 命令正在运行，非阻塞获取失败（非零退出码），Skill 先做陈旧锁检测（见下）；若确认是真实并发则报错退出：「检测到另一个 `/migrate` 命令正在运行；请等待其完成。如确认无进行中任务，可手动删除 `.rust-migration/.migration-lock`」——**不自动退避重试**（避免两个进程都通过锁后并发写 `migration-state.json`）。`flock` 在进程崩溃时由 OS 关闭 FD 自动释放，故正常崩溃无需人工清理；`[orchestration].lock_timeout_secs`（默认 3600）仅作「锁文件残留但 flock 已释放」或跨机场景的兜底（`now - started_at > 阈值` 判为陈旧）。完整 Step 0 锁协议骨架（锁文件 JSON 格式、PID 有效性检查、陈旧锁恢复、平台差异、手动逃生口）见 [09-appendix-schemas.md 附录 B Step 0](./09-appendix-schemas.md#附录-bmvp-skill-的-skillmd-骨架)。
...
> **Phase 检查点与断点续传**：translator 除写 `attempts/{module}-phase-a.rs`（Phase A 完成）外，还在 `migration-state.json` 的 `attempts[].timestamp` 记录 Phase A 完成时间戳，恢复逻辑据此判断崩溃发生在哪个 Phase；Phase B 失败时写 `attempts/{module}-phase-b-partial.rs` 并置 substatus=`phase_b_failed_at_round_N`，`/migrate run --retry` 从该检查点重入 Phase B（契约见 [03 § 4.3 Step 5](./03-execution-model.md#43-内循环模块级单会话内-phase-ab-双阶段翻译) 与 [09 附录 A substatus 约定](./09-appendix-schemas.md#附录-amigration-statejson-schema)）。
...
min_applicable_dimensions_per_function = "auto"  # auto（1-2 参数≥5 维 / 3+ 参数≥3 维）| N；§7.7 探测维度覆盖下限，verifier 自校验
...
lock_timeout_secs = 3600             # 全局命令锁陈旧兜底阈值：flock 崩溃自动释放，此值仅用于「锁文件残留但 flock 已释放」或跨机场景的 now-started_at 判定（见 § 10.5 与 09 附录 B Step 0）
...
| `porting-template.md` | Markdown（YAML frontmatter + 规则正文） | frontmatter 格式复用 [05-documentation-system.md § 6.1](./05-documentation-system.md#61-核心产出物总览)；对适配器的特殊约定：必含 `language_id`、`rule_version` 字段，至少一条 `## 类型映射` 标题 |
