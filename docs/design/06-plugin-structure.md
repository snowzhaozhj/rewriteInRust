...
| `rustmigrate state populate-modules` | 用源码图迁移序列填充 `migration-state.json` 的 `modules`/`sprint`（PLAN 操作）：读 `source-graph.db` → `migration_sequence()` 拓扑序 → 每个文件模块写 `{status:pending, sprint:1, risk:low}`（module key 用 NodeId 原值，与 `graph deps` 输出一致）+ `sprint.current=1`；有环拒绝填充。**（M2-TIER-01a：删除 M1 起恒为 `Low` 的死字段 `risk`，此处改填 `tier`，见 [03 § 4.3.2](./03-execution-model.md#432-复杂度自适应分档tier-01m2)）**是 `/migrate analyze`→`/migrate run` 衔接的 PLAN 落盘环节（见 PLAN.md §9.5） |
...
| `source-graph.db`（SQLite） | 单写者；CLI `graph build` 与 analyzer 串行 | **D3/D5 定稿：编排器集中 writer（主架构）**——翻译期图**只读**，各 worktree SubAgent 经 `[workspace] shared_db_path`（绝对路径、只读）加载独立 petgraph 副本、不直写 db；唯一写入口是 `graph build → save_to_db`，运行时状态由编排器单写 `migration-state.json`。翻译期无多 writer 并发，**WAL 并发写策略降为未来「SubAgent 直写 db」模式的可选回退**（见 [04 § 5.7.3](./04-toolchain.md#573-持久化存储) 与 [08 §M2 图写架构](./08-roadmap-and-reference.md#m2-质量提升8-12-周)） |
