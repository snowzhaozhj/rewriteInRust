...
| `rustmigrate scaffold workspace` | 生成迁移目标 Rust 项目的基础骨架（委托 `cargo init --lib --vcs none`）；dev-dependencies 与 `deny.toml` 由 scaffolder SubAgent 按项目测试需求注入。同时写 `.gitignore`（`/target`，后置条件式幂等——缺有效规则则追加、保留用户既有内容）；`cargo init --vcs none` 不产 VCS 文件，缺它会让 worktree 自检产物被 `git add -A` 吞入提交污染合并（ORCH-01 PR-5 实测）。**产出物实为单 crate（`[package]` + `src/lib.rs`），不含 `[workspace]` 段**——单 crate 输出是既定设计（见下方 § M2 写隔离约束「worktree 是并行机制，与输出 crate 结构正交，M2 沿用单 crate 输出」），命令名中的 workspace 是历史沿称，保留以免破坏 `scaffolder.md` / `workflow.md` / `SKILL.md` 的既有调用；真需要多 crate 分层时另开命令而非改本命令语义。**目标落在已有 workspace 的仓库内时会牵连用户的构建配置**：cargo 可能把新 crate 纳入外层 workspace（显式追加进 `members`，或被既有 glob 如 `crates/*` 直接覆盖——后者**不改** manifest），此后该仓库的 `cargo build`/`test` 会连带编译迁移产物（迁移中的 crate 常处于 `unimplemented!()` 等不可编译中间态）。CLI 的检测判据是**状态**而非变化量——调 `cargo metadata` 问「目标是否已在 `workspace_members` 里」（它自己解析 members/glob/`exclude`，是权威真值源）。两种更朴素的判据均经审查实证推翻：匹配 cargo stderr 文案（随版本变动、可被本地化）、比对 manifest 改动前后内容（glob 场景下 manifest 根本不变，结构上永不触发）。命中时汇入 `warnings` 并降级 `status=warning`；**不报错**（成为 member 本身无破坏、用户也可能确实想要），能否接受由用户判断。处置建议须同时提 `exclude`——仅从 `members` 移除会让 cargo 报 `current package believes it's in a workspace when it's not`。幂等重跑亦按状态重新判定（曾有「首次因 `.gitignore` 写失败报错、重跑报 ok」使改动永久隐形的漏洞）|
