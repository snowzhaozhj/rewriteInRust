...
| `BLOCKED_BY_VALIDATION_FAILED` | **语义码未落地，但校验已落地为告警**（M4：`validate state` 检出幽灵引用并降级 `status:warning`，`--check-blocked` 另在 `data.ghost_refs` 列出逐条明细 `{module, missing, status}`；不返本码、不硬判损坏——旧文件须可读） | `blocked_by` 引用了**无处归属**的 key（既非登记模块，也不是任何 composite 组的成员——成员 key 会先归一到组代表，不算幽灵） | **CLI 只检出、不处置**：`ghost_refs` 每条为纯检出事实 `{module, missing, status}`，`status=blocked` 表示此刻就永久阻塞、其余表示残留引用。按状态推导「处置命令」的那一层曾把「不可能」写进用户可见输出、而依据只是「看出边」（一步可达），被审查全数推翻，已拆出为后续 PR（见 [MDR-021](../decisions/021-no-json-schema-validation.md)），届时提供非破坏性清理入口。人工处置须守两条硬约束：⑴ **transition 不是通用处置**——`blocked_by` 只在离开 blocked 的边上清空，对非 blocked 模块 transition 要么报 `E004`、要么**返回成功而引用原样留着**（后者更危险，`status:ok` 会被误读成已处置）；⑵ **不可用 `populate-modules`**——它对非 pending 模块拒绝重填。`--auto-unblock` 对 `status=blocked` 者**不放行**（等待依赖就绪是无效动作）。reset 需 `--force` 的状态（值域取自 `machine.rs` 的 `reset_force_reason`，勿手工维护——有 CI 守卫比对）：`blocked`、`done`、`degrade_ffi`、`degrade_manual`、`degrade_skip`、`paused` |
