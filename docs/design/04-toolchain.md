...
> **just / bacon 定位说明**：just（任务自动化）和 bacon（持续编译监控）为推荐本地开发工具，不经 CLI ToolRunner 调用，用户按需安装即可。它们不属于 § 5.8 B 的程序化子进程调用范畴。
...
- **有并发模式 → 条件强制启用 loom 或 shuttle**（按并发原语类型分流）：
    - **std::sync 原语**（`Arc<Mutex>`/`RwLock`/`Condvar` 等同步并发）→ 提升 **loom**（Tier 2 → 条件强制 Tier 1），loom 重写 std::sync 原语模拟线程交织。cfg-gate 示例：`#[cfg(loom)] use loom::sync::Mutex; #[cfg(not(loom))] use std::sync::Mutex;`。
    - **tokio::sync / async 并发**（`tokio::spawn`/`tokio::sync::Mutex`/async channel 等异步并发）→ 提升 **shuttle**（Tier 2 → 条件强制 Tier 1），shuttle 通过受控调度器执行 tokio 运行时探索异步任务交织（`shuttle::check_random(|| { let rt = shuttle::runtime::Runtime::new(); rt.block_on(async { ... }) })`）。注意：shuttle 不提供 cfg-gate 式原语替换，而是在 executor 层拦截调度。
    - 两者互斥选择由 analyzer 自动判定；混合场景（同时含 std::sync 和 tokio::sync）两者均启用。
...
| **ast-grep-core** | 0.x API 不稳定（与 OXC 同标准） | 回退路径：去 ast-grep-core → tree-sitter 直接查询（+1-2 人天），详见 [08 § M0 Spike 0](./08-roadmap-and-reference.md#m0-假设验证周2-3-周) |
...
| **Leiden (社区检测)** | 无成熟纯 Rust crate | 回退选项：(a) louvain-rs（Louvain 近似，成熟度低）(b) 自实现 Leiden 核心循环（~2-4 人天）(c) 退化为目录分组（已定义，§ 5.7.4.1）。M2 启动前在 `DESIGN_ASSUMPTIONS.md` 评估当时 crate 生态。**已落地**：批次划分由 [MDR-011](../decisions/011-coupling-agglomerative-decomposition.md) 目录优先凝聚取代（不引社区检测库）；`stats community` 诊断走自实现 Louvain（无外部依赖），见 § 5.7.4.1 与本表脚注 |
...
| 语义 | `Community` | 功能聚类（Louvain 算法产出，见 `stats community`） | M2 |
| 迁移 | `RustTarget` | 对应的 Rust 目标实体（struct/enum/trait/function/module） | MVP |
| 迁移 | `TestFixture` | 黄金文件测试锚点 | MVP |

> **MVP/M2 划分原则**：一个节点类型是否 MVP，取决于它是否影响核心迁移循环（analyze → run → review）。TypeAlias 不影响翻译顺序（不引入新的模块间依赖）；Variable 跟随所在 File/Module 一起翻译；Community 需要 Louvain 算法支持。
>
> **设计决策**：
...
-- 元数据表（§5.7.5 熔断机制 & §5.7.6 topo-sort 前置检查依赖）
CREATE TABLE metadata (
    key   TEXT PRIMARY KEY,
    value TEXT NOT NULL
);
-- MVP 预置行：全量 graph build 完成后 UPSERT 重置为 'full'
INSERT INTO metadata (key, value) VALUES ('graph_integrity', 'full');
```
...
| 2. AST 解析 | tree-sitter + ast-grep-core（CLI 内嵌）+ 适配器（外部） | 源文件 | 节点 + 边（见下方工具分工） | 是 |
...
> **Stage 1 与 `profile` 命令的关系澄清**：Stage 1 的文件扫描由 `graph build` 内部完成（基于 `--root` 参数递归扫描匹配 `source_language` 扩展名的文件），不要求先调用 `rustmigrate profile`。`profile` 是独立的诊断命令（输出项目画像 + 工具可用性），供 SKILL.md Step 1 读取其 JSON 输出用于语言确认与外部工具检查，与 `graph build` 无数据依赖。上表"工具"列填写 `rustmigrate profile` 仅表示该阶段的扫描逻辑与 profile 的文件发现逻辑共用同一实现（`FileScanner` 模块），而非运行时调用依赖。

**批次划分**（借鉴 UA 社区分组思路，原设计采用社区检测算法）：

> **实现取代（[MDR-011](../decisions/011-coupling-agglomerative-decomposition.md)）**：本节以下描述的「社区检测分批 + 目录退化」是原始前瞻设计。实际 `plan-decomposition` 落地为**目录优先两阶段凝聚合并**（SCC 缩点 DAG 上沿目录/耦合双轴凝聚，保 quotient 无环），**不引 Leiden/Louvain 库**（理由见 MDR-011 §3）。以下社区检测相关的性能基准与退化算法为历史设计上下文，非当前实现。

阶段 2 对大项目（>200 文件）按 import 关系做社区检测分批，每批上限 35 文件。这让每批内的文件有上下文关联，提升 LLM 分析质量。退化方案：社区检测失败时按目录分组。性能基准、35 文件依据与退化算法见 § 5.7.4.1。

**阶段 2 工具分工**：tree-sitter 产出 `contains`/`imports`/`exports`/`extends` 边（确定性结构解析）；ast-grep-core 补充 `calls` 边（模式匹配）；适配器补充 `imports` 边（动态 import / re-export）。

**CLI `graph build` 与适配器 `extract-deps.sh` 的职责关系**：

- **`rustmigrate graph build`**（阶段 2）：使用 tree-sitter + ast-grep-core 做确定性解析，产出基础图（contains、imports、exports、extends、calls 边）。这是图的骨架，解析过程确定性（同一输入产出同一图）。
...
**算法复杂度**：社区检测（Louvain/Leiden 同阶）时间复杂度约 **O(m log n)**（m=边数、n=节点数），见 Traag et al. (2019)。import 图是接近 DAG 的稀疏图（边数与节点数同量级，m ≈ O(n)），故实际接近 O(n log n)，对 <500 文件规模可接受。（注：此为原批次划分设计的复杂度分析；批次划分已由 MDR-011 目录优先凝聚取代，`stats community` 诊断实际用 Louvain。）

**35 文件/批的依据**：该数字主要来自**上下文预算约束**而非纯经验值——按 [02 § 3.5](./02-architecture.md#35-llm-上下文窗口管理) 的 ≤100K token 预算，每文件约 2.8K token 时 35 文件 ≈ 98K token，正好贴近上限。UA 经验给出同量级参考。但「35 文件能否达成批内高内聚、低跨批重复」与「批大小 vs LLM 分析质量」两个假设未实测，须在 M0 [Spike 3](./08-roadmap-and-reference.md#m0-假设验证周2-3-周) 的「批大小优化验收」子项中量化（3-5 个中型项目 × 批大小 20/35/50），**指标用与批大小因果强的三项**：批内符号引用覆盖度、同符号跨批重复分析次数、跨批依赖链准确度（不用 proptest 等价率 / clippy 告警数，二者与批大小因果弱）。该验收为 M1 非阻断交付物（结论写入 `DESIGN_ASSUMPTIONS.md` 作为 M2 批大小调优输入），不阻断 M1 验收；若 M1 时间压力可推迟到 M2 前 2 周与上下文预算实证校验合并执行（理由：MVP 目标 <5K 行 ≈ ~33 文件，低于 >200 文件的批处理激活阈值，批大小优化对 MVP 项目无实际影响）。验收失败时的分档决策（批大小调整 / 目录降级 / 自适应分批）见 [08 § Spike 3 补充 a 决策树](./08-roadmap-and-reference.md#m0-假设验证周2-3-周)。批大小可经 `.rustmigrate.toml` 的 `[analysis] batch_size = "auto"`（或 15-50 固定值，schema 权威见 [06 § 11.1](./06-plugin-structure.md#111-rustmigratetoml-配置文件)）配置；M1 运行时监控实际跨批重复率，超过 `[analysis] batch_reuse_rate_threshold`（默认 0.30）时告警并建议启用自适应分批或目录降级。
...
> **MVP topo-sort 前置检查**：`topo-sort` 执行前读取 `source-graph.db` metadata 的 `graph_integrity` 字段——值为 `"full"` 时正常输出；值为 `"truncated_*"` 时返回非零退出码 + 诊断 JSON `{"error":"graph_truncated","truncated_at":"...","suggestion":"run rustmigrate graph build --full"}`，阻塞继续（与下方 `topo-sort` 有环返回 E002 的拒绝行为对齐）。
>
> **`topo-sort` 有环行为（纯排序原语，仍拒绝）**：`topo-sort` 用标准 Kahn 算法，是**纯排序原语**，对有环图无法产出有效排序，故仍返回 E002 `CyclicDependency` 非零退出码并列出环路径。**这是它的契约，不变**。
>
> **破环收口在 populate，不在 topo-sort（M2-SCALE-SCC，见 [MDR-004](../decisions/004-scc-fold-break-cycle.md)）**：源码循环依赖（含 re-export 间接环）**不再**拒绝填充 + 人工破环/降级。`state populate-modules` 用 Tarjan SCC 把每个强连通分量**缩点折叠为一个 composite 模块组**（`ModuleState.member_files` 列出组内全部互引源文件，module key=组内字典序最小成员），在缩点后无环的 DAG 上排 sprint 层级。**核心论据**：Rust 同一 crate 内 mod 之间允许互相 `use`（mod 间循环引用合法，只有 crate 间禁环），故源码环不是翻译障碍，无需破环/shared-types/FFI。**折叠后翻译粒度=单文件**（SCC 是编译门禁单元≠翻译单元，整组一次会撑爆上下文）：契约+stub→契约门→逐文件填空→整组编译门，详见 [MDR-006](../decisions/006-scc-per-file-stub-first.md)。
>
> **E002 / topo-sort 语义分叉（有意为之，非 bug）**：`graph topo-sort` 命令对有环图返回 E002 `CyclicDependency`，而 `populate-modules` 折叠**不拒绝**——两者语义差异是有意为之：`topo-sort` 是纯排序原语（环 = 无法排序），破环逻辑收口在 `populate-modules`（SCC 缩点）。完整 SCC 检测能力（`graph cycles`）见上表 M2 行。FFI 切分退化为「单个 SCC 大到超上下文预算」时的兜底（当前 TODO，未实现触发路径），不再是环的默认降级。详见 [MDR-004](../decisions/004-scc-fold-break-cycle.md)、[03 § 4.2 循环依赖处理](./03-execution-model.md#42-外循环sprint-级跨会话天周)。
...
| 工具 | 输入 | 输出 | 集成理由 |
|------|------|------|---------|
| **tree-sitter** + 语言绑定（tree-sitter-typescript / tree-sitter-python / tree-sitter-go 等） | 源文件路径 | AST 节点 JSON（类型、位置、子节点） | 多语言 AST 解析是核心能力，必须零依赖可用 |
| **ast-grep-core** | AST + 模式规则 | 匹配结果 JSON（位置、捕获） | 代码搜索/重写是高频操作，嵌入避免 CLI 调用开销。**注**：0.x API 不稳定风险见 § 5.5 TRIAL |
...
> **Go 工具链（M4 Go 线，v0.9.4）**：Go 解析锁 `tree-sitter-go=0.21.x`（GO-10 grammar 契约测试防节点漂移）。Go 包系统「目录=package」——`resolve_import` 经扩 trait 的 `list_dir` 回调枚举包目录取代表文件（字典序第一非 `_test.go`），`configure_project` 读 `go.mod` module 前缀（D-M4-01，baseline 非 fallback）。`_test.go` 与非默认平台后缀（`_windows.go` 等）+ `//go:build` 门控排除。并发（go/select/chan）/反射（reflect）/cgo（`import "C"`）/unsafe 触发 danger 分类 → translator 默认降级 + 推荐 crate（D-M4-05）。interface 隐式实现不强连 Implements 边、标注方法集供 LLM 判断（D-M4-02）。Go 图分析/tier 判定/classify 阶段无独立外部工具（不依赖 `go vet`/`staticcheck`）——全部经嵌入 tree-sitter 完成，与 TS/Python 同架构（验证阶段可选调用 `go version` 见 `adapters/go/analysis-tools.json`，`required:false`）。
