...
| **Leiden (社区检测)** | 无成熟纯 Rust crate | 回退选项：(a) louvain-rs（Louvain 近似，成熟度低）(b) 自实现 Leiden 核心循环（~2-4 人天）(c) 退化为目录分组（已定义，§ 5.7.4.1）。M2 启动前在 `DESIGN_ASSUMPTIONS.md` 评估当时 crate 生态 |
