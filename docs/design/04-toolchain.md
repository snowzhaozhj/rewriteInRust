...
> **Go 工具链（M4 Go 线，v0.9.4）**：Go 解析锁 `tree-sitter-go=0.21.x`（GO-10 grammar 契约测试防节点漂移）。Go 包系统「目录=package」——`resolve_import` 经扩 trait 的 `list_dir` 回调枚举包目录取代表文件（字典序第一非 `_test.go`），`configure_project` 读 `go.mod` module 前缀（D-M4-01，baseline 非 fallback）。`_test.go` 与非默认平台后缀（`_windows.go` 等）+ `//go:build` 门控排除。并发（go/select/chan）/反射（reflect）/cgo（`import "C"`）/unsafe 触发 danger 分类 → translator 默认降级 + 推荐 crate（D-M4-05）。interface 隐式实现不强连 Implements 边、标注方法集供 LLM 判断（D-M4-02）。Go 无独立外部工具（不依赖 `go vet`/`staticcheck`）——全部经嵌入 tree-sitter 完成，与 TS/Python 同架构。
