...
**批次划分**（借鉴 UA 社区分组思路，采用 Leiden 算法）：
...
**算法复杂度**：Leiden 社区检测时间复杂度约 **O(m log n)**（m=边数、n=节点数，与 Louvain 同阶），见 Traag et al. (2019)。import 图是接近 DAG 的稀疏图（边数与节点数同量级，m ≈ O(n)），故实际接近 O(n log n)，对 <500 文件规模可接受。
...
| `community` | 功能聚类分析 | Leiden 社区检测 | 否 |
