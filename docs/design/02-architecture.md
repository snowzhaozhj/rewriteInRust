...
> **Phase A/B 与状态机的映射**：状态机图中的 `TRANSLATE` 是单一概念状态，但其内部包含 [03 § 4.3](./03-execution-model.md#43-内循环模块级单会话内-phase-ab-双阶段翻译) 的 Phase A（忠实翻译）→ 对抗性审查 → Phase B（惯用化优化）三个子步骤。Phase A/B 是 `TRANSLATE` 状态内的**内部子步骤**，状态转换在 Phase B 首次产出代码后发生：`cargo check` 通过则 `translating` → `testing`；失败则 `translating` → `compile_fixing` 进入重试循环，通过后 `compile_fixing` → `testing` → `reviewing`（映射见 [09 附录](./09-appendix-schemas.md#状态机概念名--json-字段值映射)）。对抗性审查在 TRANSLATE 内部执行（Step 3），不触发状态转移；VERIFY 的 `reviewing` 子状态为测试通过后的最终签批门禁（TODO(port) 清零 + 验收）——**该门坐落在 `reviewing → done` 边本身**，三种处境：`reviewing + agent_done`（agent 自检过、待整组验证/送审）、`reviewing + awaiting_final_review`（**已停门等人签批**，断点续跑据此路由到「展示证据包 + 等签批」而不重跑 verifier）、`reviewing` 其他 substatus（流程未走完 → 强制人工）。裸 `state transition --to done` 被 CLI 拒绝，唯一入口 `state approve`（人签批 / 用户预签窄策略 + 编排器逐项 attest）；规范见 [03 § 4.3.3](./03-execution-model.md) 与 [MDR-019](../decisions/019-post-translation-review-gate.md)。
>
