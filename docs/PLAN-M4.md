...
- **并行编排程序化调度器——推迟（但并行本身要建）**：并行基础设施（`parallel.rs` / `topo.parallel_groups` / `run.md` 并行段 / `workflow.md`）**仅 coded-but-dead——从未接进活路径、从未端到端跑通**（见 [MDR-018](decisions/018-keep-parallel-migration.md) 现状基线）。**程序化状态机调度器**（用 Rust 状态机取代 SKILL.md 散文编排）投入大、ROI 不足，**推迟**；但**并行翻译 + worktree 写隔离本身是核心能力、须真正落地**——Sprint F ORCH-01 由「worktree 生命周期代码化」重定义为「并行 + worktree 编排真正接入活路径并跑通」（工作量待专门规划，见 MDR-018）+ **循环健壮性大幅扩充**（ROB-01a/b/c 共 3d，覆盖 M3 实际遇到的 watchdog stall、额度耗尽、单模块失败扩散等可靠性问题）。
...
- 并行编排基础设施**仅 coded-but-dead**（`parallel.rs` / `topo.rs:compute_parallel_groups` 已编码但无生产消费者、`parallel_groups` 不落盘、无 E2E——见 [MDR-018](decisions/018-keep-parallel-migration.md)）→ ORCH-01 须接入活路径
...
- [~] jmespath + 1 TS 项目质量基线报告落地（`docs/m4-quality-baseline.md`）—— **部分完成**：报告文档已落地（[m4-quality-baseline.md](m4-quality-baseline.md)），汇编 Go（semver/go-humanize，`stats quality` CLI 实测 final_score=100）+ Python（jmespath 901/902、textdistance 70/70 强等价实测换算）横比起点。**字面子项「1 TS 项目」未达成**：M3/M4 真实迁移全用 Python/Go，TS 仅 fixture 级——留待真实 TS 项目迁移补齐（~1.5d），不阻断横比起点建立。Python 侧无 CLI 数值（M3 迁移早于框架落地、产物已清空）亦如实标注
...
| M4-ORCH-01（重定义） | **并行 + worktree 编排真正落地**（原「worktree 生命周期代码化」，见 [MDR-018](decisions/018-keep-parallel-migration.md)）：并行基础设施 coded-but-dead → 接 `parallel_groups` 进 CLI/state 输出、两套 worktree 机制择一删另一套、建逐层合并 + reconcile + 整组 `cargo check` 真门编排、补端到端集成测试。**不含程序化状态机调度器**（推迟） | 待专门规划估时 | — |
...
| F 健壮性 + 编排收口 | 巩固 | 5 | 5.5d+（ORCH-01 待重估） | checkpoint 硬化 + watchdog stall 恢复 + 额度韧性续跑 + 并行+worktree 真正落地（ORCH-01 重定义，MDR-018）+ 版本陈旧检测 |
...
| 并行编排**程序化调度器** | **仅调度器推迟，并行本身要建**（见 [MDR-018](decisions/018-keep-parallel-migration.md)）：SKILL.md 散文编排满足调度需求，Rust 状态机调度器投入大、当前 ROI 不足。M4 做 ORCH-01 并行+worktree 真正落地 + **循环健壮性大幅扩充**（ROB-01a/b/c 共 3d：checkpoint 硬化 + watchdog stall 恢复 + 额度韧性续跑）；**推迟**程序化调度器 |
...
| 多 agent 并行编排优化 | ✅ **保留并真正落地**（[MDR-018](decisions/018-keep-parallel-migration.md)） | Sprint F ORCH-01 并行+worktree 真正接入活路径 + ROB-01a/b/c 循环健壮性 3d（checkpoint/watchdog/额度韧性）；程序化调度器推迟 |
