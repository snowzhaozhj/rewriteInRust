...
> **⚠️ M2 详细实施计划已迁移到 [`docs/PLAN-M2.md`](PLAN-M2.md)**。
> 本节保留原始方向级规划作为历史记录。新会话应直接读取 PLAN-M2.md。
> PLAN-M2.md 经过 6 个 subAgent 深度分析（代码盘点 + 设计文档交叉核查 + 依赖分析 + 可行性评估）和 2 轮对抗审查产出。
...
| M2-VER-04 | populate 数据卫生 + 契约加固（PR #10 审查遗留） | ① populate 前清理不在新 sequence 的孤儿 pending 条目 ② docstring "整体重填"改为"按 NodeId 合并" ③ record-subagent-call 无 init 错误路径 e2e |
...
> **M1 遗留设计决策（需定夺后可能产生额外任务）**：
> - `done + --force` 重做：设计行 379（暗示可重做）vs 行 209 矩阵（done 硬终态）矛盾——定义 done 是否真终态
> - blocked 进入：设计行 206「可从任何状态进入」vs 矩阵（仅 blockable 活跃态可进）——需统一规则
> - per-module risk 评估：当前 `ModuleState.risk` 恒 Low 未填，M2-TIER-01 分档依赖此信号——需 analyzer 增产或 populate 时自动评估
...
| M2-ADV-07 | 默认 TODO 决策策略（headless 模式） | headless 撞 TODO(port) 时按 safe-default 自动决策，不卡 |
...
| 方向 | 内容 | 状态 |
|------|------|------|
| C/C++ 适配器 | tree-sitter-c + unsafe Rust 翻译 | ⏸️ 推迟（D-M4-03，语义难度+ROI） |
| Go 适配器 | tree-sitter-go + 并发模型映射 | ✅ 完成（Sprint C/D/E，端到端验收达标） |
| Kani 形式验证 | 关键路径数学等价性证明 | ⏸️ 推迟（D-M4-04，与 proptest 互补，当前 ROI 不足） |
| 社区生态 | 文档 + 示例 + Plugin marketplace 发布 | 部分（GOV-01 版本检测 + QUAL-04 结构诊断；运营非代码范围） |

> M4 详细任务分解、决策记录（D-M4-01~06）与验收见 [PLAN-M4.md](PLAN-M4.md)。
