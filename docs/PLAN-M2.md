...
**影响**：
- M2-SCALE-02 = 「git worktree 编排 + SubAgent worktree 内完整自检 + 禁改共享文件/共享变更回传串行 + dependency-mapping 前置 + merge 后整体 check 真门 + 图缺陷回退串行」（§7 同步重写）
...
| M2-DESIGN-03 | 设计文档架构同步：04 §5.7.3 / 08 §M2 并发写架构 → D3 worktree 下编排器集中 writer（D5）+ **06 §10.5 worktree 措辞本就正确无需改**，仅补「dependency-mapping 前置生态约束 + 禁改共享文件/共享变更串行」（D3 约束包）+ 补 TIER-01 分档定义章节（D4） | docs/design/ | 0.5d | — |
