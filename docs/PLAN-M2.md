...
| SQLite WAL 配置回归 | `journal_mode=WAL` + `busy_timeout` 连接配置正确 | 配置断言测试（方案B 集中 writer，**无并发写门禁**，见 §2 D5） |
...
**推荐决策**：**方案 B 修正版 — SubAgent 隔离完整 crate 内自 check + 写磁盘不回传代码 + 编排器 toml 合并**。
> 决策演进（2026-06-15）：原 6-Agent 选方案 B → 复审一度改 worktree（理由：LLM 编排器上下文成本）→ **codex 对抗审查推翻 worktree**：该论证基于稻草人——`translator.md:100-105` 已要求 SubAgent 把 Rust 文件**写磁盘**（非回传对话），故方案 B 本就不撑爆上下文。worktree 反而引入真硬伤（dirty tree merge、多 crate workspace 边界、git 操作链）。最终取「修正版方案 B」：保留 B 的隔离目录+统一 merge 骨架，修正两点表述/实现。

**关键前提**：M2 编排器是 SKILL.md（**LLM 驱动**，§11 程序化推迟 M2.5）。LLM 编排器上下文预算是硬约束——修正版的核心就是确保产出物落盘、编排器只收摘要。

- **方案要点**：
  1. **隔离目录升级为完整 crate**：每模块隔离目录是一份 `scaffold` 产出的完整 lib crate 副本（含自己的 `Cargo.toml`，A3 已验证 `cargo init --lib` 产可独立 `cargo check` 的 crate）。
  2. **SubAgent 自 check 闭环**：SubAgent 在隔离 crate 内跑 `translate→cargo check→compile_fix→test`，**保留 M1 per-module 循环**；代码/rustc 错误/修复全程在 SubAgent 子上下文。
  3. **写磁盘不回传代码**：SubAgent 只回传 `{module, files_written, new_deps, check_status}`——**纠正原 D3 行110「返回 rust_code」表述**（那会误导为回传代码内容）。这是上下文经济的关键。
  4. **编排器统一 merge + 兜底 check**：复制 `files_written` → 主 `rust_root/src/` + 程序化合并 `new_deps` → 主 `Cargo.toml`（deps 取并集，同 crate 版本取高/不兼容报错，A2 验证并集可行）+ lib.rs mod 声明统一追加 + 统一 `cargo check`/`cargo test` 兜底（捕获跨模块类型冲突）。
- **为何不用 worktree**：worktree 仅多解决 Git 隔离，却带来 dirty tree merge 复杂度、多 crate workspace 边界（scaffolder 当前实为单 crate）、git add/remove/merge 操作链——隔离目录(普通目录复制)更简单且同样满足上下文经济+自 check。
- **未解点（实现期定夺）**：① **M2 并发 agent 数量未指定**（默认 `max_concurrent_agents=3`）——决定 rust_root 写冲突的退化条件 ② 并发写同一 `rust_root/src/`：不同模块写不同文件无冲突，`Cargo.toml`/lib.rs 由编排器统一写故无并发——若未来真冲突则退化为分时合并 ③ 共享 vs 独立 `CARGO_TARGET_DIR`（cargo target 锁行为 codex 因沙箱未实测，Sprint F 实测调参）。

**影响**：
- M2-SCALE-02 = 「隔离完整 crate + SubAgent 自 check + 编排器复制 merge + toml 程序合并 + 统一 check 兜底」（§7 同步修正）
- 设计文档：04 §5.7.3 `shared_db_path`（只读）仍适用；08 §M2「多 agent 工作区」措辞改为「隔离 crate + 编排器 merge」
...
**矛盾**：08 §M2（line 169/181/182/187）+ 04 §5.7.3（line 393-399）以「多 agent 共享 `source-graph.db` 并发写 + WAL 串行化」为 M2 默认架构，并把「SQLite 冲突率<10% 且锁等待≤20ms」列为 M2→M3 硬性升级判据(c)。但 D3 选定方案B（编排器单写 `migration-state.json` + SubAgent 隔离 crate，graph 翻译期只读），**并发写场景不存在**，该门禁失去测试对象。复审草稿直接删 WAL 测试，却留下 §1.2/§1.3/F5/Level5 仍要求该项的自相矛盾。
...
- 同步更新设计文档 04 §5.7.3 / 08 §M2：标注「方案B 集中 writer 取代原共享 DB 并发写架构；原并发写策略保留为未来 worktree 模式可选回退」——并入 M2-DESIGN-03。
...
| M2-DESIGN-03 | 设计文档架构同步：04 §5.7.3 / 08 §M2 并发写架构 → 集中 writer（D5）+ 06 §10.5 / 08「多 agent worktree」措辞 → 隔离 crate+编排器 merge（D3）+ 补 TIER-01 分档定义章节（D4） | docs/design/ | 0.5d | — |
...
| 任务 ID | 内容 | 文件 | 工作量 | 依赖 |
|---------|------|------|--------|------|
| M2-SCALE-02 | 写隔离实现（D3 修正版方案 B）：隔离完整 crate + SubAgent 自 `cargo check` 闭环 + 写磁盘回传摘要（不回传代码）+ 编排器复制 merge + toml 程序合并（版本取高/冲突报错）+ 统一 check 兜底 | run.md, translator.md, scaffold/ | 5-7d | Sprint C SCALE-SPRINT |
...
**写隔离实现方案（D3 修正版方案 B 决策执行）**：

```
编排器（SKILL.md run，LLM 驱动）
  ├── 获取 sprint N 的模块组
  ├── 为每个模块创建隔离【完整 crate】 .rust-migration/workspace/{module}/
  │     （scaffold 产出的独立 lib crate 副本，含自己的 Cargo.toml，可独立 cargo check）
  ├── Agent tool 并发启动 SubAgent（max_concurrent_agents=3）
  │   ├── SubAgent 在隔离 crate 内：translate → cargo check → compile_fix → test（保留 M1 per-module 循环）
  │   ├── 代码 / rustc 错误 / 修复全程在 SubAgent 子上下文（写磁盘，不回传代码内容）
  │   └── SubAgent 仅回传摘要：{module, files_written:[...], new_deps:[...], check_status, test_status}
  ├── 编排器收集所有 SubAgent 摘要（只收摘要，上下文经济）
  ├── 统一 merge（编排器单写，无并发）：
  │   ├── 复制 files_written → 主 rust_root/src/（不同模块写不同文件，无冲突）
  │   ├── 程序化合并 new_deps → 主 Cargo.toml（并集；同 crate 版本取高，不兼容报错）
  │   ├── lib.rs mod 声明统一追加
  │   └── 复制测试文件 → 主 rust_root/tests/
  ├── 兜底 cargo check / cargo test（整组验证，捕获跨模块类型冲突）
  │   ├── 成功 → 编排器统一更新 migration-state.json（单写，无并发）
  │   └── 失败 → compile_fixing 子流程（见下方）
  └── sprint 推进检查
```

> **关键区分（消解 D3 引用 translator.md 与本节的表面冲突）**：SubAgent **写自己隔离 crate 的 Cargo.toml**（自 check 必需），但**不写主 Cargo.toml**；主 Cargo.toml 由编排器据 `new_deps` 程序化合并。translator.md:100-102 要求「更新 Cargo.toml」与 codex 指出的本节「不直接写 Cargo.toml」作用于**不同的 Cargo.toml**，二者不冲突。M2-SCALE-02 须同步在 translator.md 标注此区分。

**compile_fixing 子流程（方案 B 特有）**：
```
merge 后兜底 cargo check 失败
  ├── 编排器解析 rustc 错误输出，提取失败文件→模块映射
  ├── 尝试定位失败模块（错误发生在哪个模块的代码中）
  │   ├── 可定位：仅回滚该模块代码（从隔离 crate 恢复），其他模块保留
  │   └── 不可定位（跨模块类型冲突等）：整组回滚
  ├── 对失败模块重新启动 SubAgent 进入 compile_fixing
  │   ├── SubAgent 收到：rustc 错误输出 + 当前主 Cargo.toml deps 快照（代码已在其隔离 crate）
  │   ├── SubAgent 在隔离 crate 内修复 + 自 cargo check，回传修复后摘要（不回传代码）
  │   └── 编排器重新 merge → 再次兜底 cargo check
