...
PLG-03/04 文档 4 视角（主审/设计契约/专项/异构）+ PLG-05/06 代码 4 视角（主审/设计契约/专项测试覆盖/异构）。

- **PLG-03/04**：2 important（recover/panic 三处矛盾对齐 porting-template RULE-3、analyzer「备注」无 schema 字段改走 gaps.dynamic_features）+ 5 nit 全修。
- **PLG-05/06 代码**：主审无 important（可合并）、设计契约 4/4 PASS、异构无 bug、专项 2 important（测试增强）。落实：
  - 补 `select`→`[Concurrency]` + 分组 import`(reflect+unsafe)`→`[DynamicReflection,Ffi]` category 级测试（专项 important）。
  - `channel_type` 命中函数签名/struct 字段的 false positive（主审 nit + 异构 minor）→ **保留有意保守**，代码注释明确（channel 存在即并发相邻 + 与 detect_tier 口径统一，收窄会致分裂）。
  - danger 注释「字典序」精确为「BTreeSet 判别序，组并集另按 as_str 重排」（主审 nit）。

**后续 TODO（记账，非阻塞）**：
- **tier/danger 文件基准分裂**（设计契约 pre-existing，MDR-011:139 归档）：省略 `--root` 且 `cfg.source_root≠"."`（如 `"src"`）时，`detect_tiers_for_modules` 用 `root/cwd` 基准、danger 循环用 `source_root` 基准——tier 在 CWD 找不到文件静默跳过（tier=None）。best-effort 不产错，留待 populate 与 graph build 共享源码根元数据时一并收口。
- **语法错误文件 tier=Full 但 danger=[]**（主审 nit-2）：detect_tier 悲观 Full、classify_file 乐观空——各自安全（破损源码编译门会拦），degrade_skip 消费方对「full+空 danger」按无特定危险处理符合预期。
