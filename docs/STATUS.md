...
- **进行中 PR [#86](https://github.com/snowzhaozhj/rewriteInRust/pull/86)（收口 #85 三项记账 TODO，2026-07-29，**审查全闭环、待用户验收**）**：三项均为「说法与实现不符」，其中 ② 含破坏性变更。`just ci` 全绿（基线 809 → 819+）；远端 CI 5 项全过。
  - **① 06 命令表 ↔ CLI 一致性守卫**（闭 #85 记账 ③，也是 #85 漂移的**根因**——该表是 CLAUDE.md 定的唯一权威却靠人工维护）：新增 `cli/crates/cli/tests/design_command_table.rs`（3 测试）——`CommandFactory` 遍历真实 clap 树取叶子命令，解析 06 两段表格首列，双向断言 + 表头计数一致。**三向负向实证**（非推断）：删表行 → 报 missing；加幽灵行 → 报 ghost；两者均使表头「30 个」vs 实际 29/31 行独立报错。另有解析器自守卫测试（`>=30` 条 + 三层级锚点命令）防「空集比空集」假绿。
