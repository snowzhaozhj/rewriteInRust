...
- **PR [#88](https://github.com/snowzhaozhj/rewriteInRust/pull/88)「取消 JSON Schema 空头承诺」待用户拍板**（收口记账 TODO ③，分支 `fix/m4-jsonschema-truthfulness`，7 commits，**4 视角审查已全部闭环**，`just ci` 全绿 861 测试）。方向由实证定为 **⒜ 如实化 + 摘依赖**，非 ⒝ 补实现——决策与全部实证过程见 [MDR-021](decisions/021-no-json-schema-validation.md)。
