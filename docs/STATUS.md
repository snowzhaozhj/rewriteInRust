...
- **SKILL.md ↔ CLI 命令清单守卫 ✅ 交付**（2026-08-08，独立 PR，从 master 起）——收口 #86 记账 TODO ①。`06` 那边由 #86 钉住，但 `06:105` 表头同时要求同步 SKILL.md 清单、`SKILL.md:31` 自称「已穷举顶层子命令」，此前**零自动化检查**（#85 那次是一次性人工验证）。编排器直接读 SKILL.md，漂了就照抄不存在的命令。新增 4 测试（2 守卫 + 2 解析器合成字符串单测），追加进既有 `design_command_table.rs` 以复用 `cli_leaf_commands()`。
  ...
  - **#86 记账预警的两个坑，实测后一个成立、一个不成立**（按实测写，不照抄记账）：⒜ **成立且比记账更严重**——按行格式（`- **X**：`）全文件匹配会命中 **13 行**，其中 5 个（`守护`/`恢复`/`幂等`/`待签批`）是文档别处的同格式段落，会拖出大批伪幽灵。故须以「命令清单 + 已穷举」锚点行定位区块、按**缩进量**收尾（紧随其后就有一个同格式但缩进为零的 `profile --adapter-tools` 一级列表项）。⒝ **不成立**——记账预警「`graph export [--format json|dot|mermaid]` 的 `json|dot|mermaid]` 会残留成命令名一部分」，实测该风险只存在于「正则替换占位符」的实现；**逐 token 在首个占位符处 break** 时管道段落在 break 之后、根本到不了。已在代码注释如实记录该实测结论。
