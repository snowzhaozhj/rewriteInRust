...
- **`blocked_by` 幽灵引用检出已交付（分支 `fix/m4-jsonschema-truthfulness`，堆叠在 #88 之上，待用户拍板）**——收口 [MDR-021](decisions/021-no-json-schema-validation.md) 待办 1，是该批待办里唯一的**真实功能缺口**（其余为文档/守卫类）。`just ci` 全绿 872 测试（861 → 872）。
  ...
  - **交付**：`BlockedCheckResult` 加 `missing` 字段（`unresolved` 的子集——幽灵引用仍须阻塞，否则 `--auto-unblock` 会在损坏数据上改状态）；`validate_state` 加告警扫描，覆盖**全部模块**而非仅 blocked（正常路径离开 blocked 会清空 `blocked_by`，但手工编辑/旧文件可能残留，该模块再被标 blocked 即踩坑）；CLI `--check-blocked` 输出 `data.ghost_refs`。**破坏性变更**：`BlockedCheckResult` 新增 pub 字段，外部 struct-literal 构造会编译失败（同 MDR-020 先例，仓内构造点仅 1 处、CLI 侧只读字段，0.x 阶段不走 deprecation 期）。
  - **自查抓到的真实缺陷（本 PR 引入，已修）**：初版判据「不在 `modules` 即幽灵」会把 **composite 组的非代表成员 key 误报**——decompose 折叠后只登记组代表，成员不进 `modules` 是既定设计，而告警给的「重跑 `graph build` + `populate-modules`」对该场景是**无效动作**。危害不止误报：归一后语义完全不同（成员属哪个组、该组是否终态决定它是否就绪），不归一则组进 `done` 后引用方**永远解除不了**。修法是新增 `canonicalize_blocked_ref`（先查 `modules`，再反查 `member_files`），与 `state deps` 的归一同源、两处共用同一判据。设计契约视角独立复现了同一缺陷。
  - **四视角审查**：设计契约（5 项逐条核对，CLI 输出契约/schema 不受影响/命名规避均 PASS，1 DEVIATION 已修——破坏性变更登记位置按 MDR-020 先例移入「变更性质登记」段）+ 异构交叉（codex，3 imp）已闭环；主审与专项进行中。
    - **imp【本 PR 引入，已修】`ghost_refs` 与告警覆盖口径不一致**：告警扫全部模块、`ghost_refs` 却只从 `check_blocked_modules`（仅 blocked 模块）提取 → 非 blocked 模块残留幽灵引用时输出「`warnings` 有、`ghost_refs` 是空数组」，只消费机读字段的编排器**直接漏掉**。改为两侧共用 `scan_ghost_references`。同批修告警的失实断言：原文对 `pending` 模块也说「引用方将永久阻塞」，而它此刻并未阻塞 → 新增 `module_blocked` 区分「现在就阻塞」与「残留、将来才阻塞」。
    ...
    - **nit【已修】告警里的 key 改 JSON 字面量转义**：key 可被手工编辑，含反引号/换行时裸反引号包裹会让条目边界被内容**伪造**。
    - **codex 实证确认无问题的项**：`state recover`/`reset`/`resume`/`batch-transition-done` 遇幽灵引用行为均安全（各自拒绝或不推进）；正常离开 blocked 会清字段、无噪声告警；环检测不因幽灵边漏判或造假环；性能实测近线性（10 万模块 1.11s，非 O(n²)）；`blocked_by` 唯一正常写入路径是 `run.md` 回填 `state deps` 的 `blocking`，该输出已过归一且只放真实 key。
  - **负向实证五轮**（独立 worktree，非推断）：① `missing` 恒空（退回二分）→ 2 core + 1 e2e 报红；② 摘掉告警扫描 → 2 core 报红；③ `missing` 不计入 `unresolved`（幽灵变可就绪）→ 3 core 报红，归因精确到「带幽灵引用的模块不得判为就绪」；④ 判据反转（合法引用当幽灵）→ 3 core 报红，「反向不误报」由独立测试钉住；⑤ 摘掉 `member_files` 归一 → 2 个 composite 测试报红。
...
- **PR [#88](https://github.com/snowzhaozhj/rewriteInRust/pull/88)「取消 JSON Schema 空头承诺」待用户拍板**（收口记账 TODO ③，7 commits，**4 视角审查已全部闭环**，CI 5 项全绿）。方向由实证定为 **⒜ 如实化 + 摘依赖**，非 ⒝ 补实现——决策与全部实证过程见 [MDR-021](decisions/021-no-json-schema-validation.md)。
  ...
  - **文档如实化 10 处**（`03`/`04`/`05`/`06`/`09`/`README`）；保留 `06:889/896`（MDR-009 沿革叙述，正确）与 `08:123`（未落地路线图估算，非现状声称）。**待办 4 项**记入 MDR-021：幽灵 `blocked_by` 无检出（真实缺口）/ `E002` 定义了但不可达 / 字符串空值无校验 / `[validation]` 配置段未落地。
- **记账 TODO 余下一项**：① SKILL.md ↔ CLI 命令清单守卫（当下 35/35 准确，属未来漂移风险；可行性与两个抽取器坑位已在 #86 记账中摸清）。
