...
- **位置（2026-08-13，本条最新）**：master 在 `dac8950`——**[#90](https://github.com/snowzhaozhj/rewriteInRust/pull/90) 已由用户拍板合并**（squash，13 文件 +1191/−34），**开放 PR / 开放 issue 均清零**。#90 = `state repair --clear-ghost-blocked-by`，收口「幽灵引用检出已交付、处置为零」这个状态。决策与边界见 **[MDR-022](decisions/022-ghost-blocked-by-repair-boundary.md)**。
  - **下一步（未拍板，两个候选都来自 #90 的审查产出，见 MDR-022 待办 4/5）**：① **待办 4 优先**——`resolve_blocked_ref` / `canonical_module_key` 的早返回让「被引 key 既是登记模块、又被别组列为 `member_files` 成员」这类跨组互斥破坏判成合法，实测后果是**数据损坏**（`--auto-unblock` 真的解除引用方并落盘），是当前已知记账里唯一的正确性缺陷；② 待办 5 是同一函数的二次复杂度（2 万模块 14.2s）。两者同源、可合成一个 PR。其余记账：MDR-021 待办 2（`E002` 不可达）/ 3（`transition_inner` 下沉不变量）/ 4（字符串空值）/ 5（`[validation]` 配置段）/ 6（`--auto-unblock` 遇环丢诊断）、#87 记账 ⒜⒝、QUAL-02 的 TS 真实基线（PLAN-M4 唯一 `[~]`）。
  ...
  - **三条不变量**：① 只删幽灵条目，`Resolved`（能归一到宿主模块的引用，与宿主是否终态无关）与 `Ambiguous`（宿主歧义）原样保留，不清整个数组——保留 `Ambiguous` 是硬要求，因为对「被引 key 非登记模块而被 ≥2 组列为成员」这一形态它是跨组破坏的唯一检出通道（**另一形态检不出，见下方待办 4**）；② 不改状态、不清进度字段、不发删产物指令（与 `reset` 正相反），恢复交既有 `--auto-unblock`；③ `RepairedModule` 只放字段不放谓词，锚点判定留在 CLI 层（塞复合谓词进数据结构正是前一轮出错的形状）。
  ...
  - **四视角审查已跑并全部闭环（2026-08-13）**，每条编排器独立复核后才动手。**核心必修由 3/3 视角各自独立复现**：锚点告警的就绪判据用了错的代理——`blocked_by_remaining.is_empty()` 不等于「下一步会被 `--auto-unblock` 接手」，因为 `ready = unresolved.is_empty()` 而**已终态的剩余依赖不计入 `unresolved`**。故 `blocked_by=[幽灵, 已 done 的依赖]` 这个形状清完后剩余非空 → 旧判据跳过告警，而模块立刻就是 `ready_to_unblock`，下一步 `--auto-unblock` 必报「非法状态转换: blocked → done」。**修法：直接调 `check_blocked_modules`，不给已有判据造近似代理**（与「一个 bool 扛多个谓词」同源的教训）。另修两条用户可见文案失实：⒜ `advice` 里的 backup 路径少了前导点（实际 `.migration-state.json.backup`，`sibling_with_suffix` 会前置点），且无条件断言备份可用——按 MDR-021:81 的先例（E010 刻意不提「从备份恢复」）**整句删除**而非修成条件句；⒝ 「或经 `state update` 直接置 blocked」是假归因，该路径走 `transition_inner` 必写锚点。
  - **测试补了两条缺失边界**（都是变异实证暴露的零覆盖）：① 合法锚点须静默——把告警守卫改成恒真时 897 个测试全过，即「给每个正常模块多报一条虚假告警」可以直接上线；② noop 真的不落盘——改成恒落盘时 11 个 repair 测试全过（旧断言只比内容，而 noop 落盘写出的内容本就相同），改用 `.backup` 内容观测。另把三处手写 argv 全改为从 `REPAIR_GHOST_COMMAND` 派生（`--module` 形态此前会独立漂）。
  - **⚠️ 异构交叉（codex）抓出两条 pre-existing，判为独立 PR，已记 MDR-022 待办 4/5**：① **`resolve_blocked_ref` 的歧义判定漏一整类形态**——「被引 key 既是登记模块、又被别组列为 `member_files` 成员」时早返回判 `Resolved` 而非 `Ambiguous`。**编排器独立复现，后果是数据损坏不止漏诊断**：`shared.ts` 登记 `done` 而 `g1` 是 `translating` 时，`validate state` 零跨组告警、`--auto-unblock` **真的解除**了引用方并落盘。`canonical_module_key` 同型早返回、同样漏。这条也**推翻了 MDR-022 决策 2「Ambiguous 是唯一检出通道」的普适性**，该表述已收窄为「只对『被引 key 非登记模块而被 ≥2 组列为成员』这一形态成立」。② 全为幽灵的坏 state 上 `resolve_blocked_ref` 呈二次复杂度（1000 模块 0.72s / 10000 模块 4.55s / **20000 模块 14.2s**）——与 MDR-021 记的「10 万模块 1.11s 近线性」不矛盾，那测的是合法引用（哈希命中即返回），这测的是全未命中路径。两条都改 #88 检出层的既有判定/实现，须自带测试与负向实证，塞进本 PR 恰是「审查面过大」的复发。
  - **已合并（2026-08-13，用户拍板）**：squash 到 master `dac8950`。合并前远端 CI 针对 head `8e5d5de` 复跑 5 项全过；本地 `just ci` 在最终代码上全绿 **897 测试**（886 → 897）。**负向实证累计 11 轮**（独立 worktree，全部报红且归因精确）——功能层 7 轮见 PR 正文，审查修复层 4 轮：⑧ 就绪判据退回 `blocked_by_remaining.is_empty()`（复现三视角抓到的漏报）→ 锚点测试红；⑨ 告警守卫恒真 → 同一测试红（合法锚点须静默那一侧）；⑩ noop 也落盘 → 幂等测试红（`.backup` 被覆盖）；⑪ noop 回显退回原始入参 → 幂等测试红。
- **位置（2026-08-12，已被上条取代，保留作历史）**：master 在 `eb94959`——**[#88](https://github.com/snowzhaozhj/rewriteInRust/pull/88) 已由用户拍板合并**（squash），**开放 PR / 开放 issue 均清零**。**#88 已按第四轮结论收窄为「幽灵引用检出层」**——第三轮补的**处方层**（`GhostRemedy`/`remedy`/状态网格/四类处置文案）被第四轮证明仍把「不可能」写进用户可见文案（依据只是「看出边」，而 `X→blocked→X` 两步往返零代价清 `blocked_by` 且保留全部进度、`degrade_*` 多步可达等反例存在），故**整体移出 #88、拆为后续 PR**（届时设计非破坏性入口 `state repair --clear-ghost-blocked-by`）。详见 [MDR-021](decisions/021-no-json-schema-validation.md)「第四轮 + 拆 PR」段。
  ...
  - **下一步（用户 2026-08-12 拍板）**：做**处方层 PR**——设计非破坏性入口 `state repair --clear-ghost-blocked-by`，收口「检出已交付、处置为零」这个状态（当前告警只能如实写「当前 CLI 未提供自动处置命令」）。**全部约束已由第四轮实证，勿再当新发现**，见 MDR-021 文末「遗留给后续处方 PR 的约束」：两步往返依赖待办 3 的 `transition_inner` 宽松 / `reset` 清整个 `blocked_by` 数组会连带擦掉同模块的 `Ambiguous` 引用（`member_files` 跨组破坏的唯一检出通道）/ 非法锚点、`graduate`+`blocked`、`degrade_*` 多步可达都必须在**可达性**而非「一步出边」层面处理。
