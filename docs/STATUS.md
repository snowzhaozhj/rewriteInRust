...
- **位置（2026-08-11）**：master 在 `e9220bf`（#89 已合并），**开放 PR 仅 [#88](https://github.com/snowzhaozhj/rewriteInRust/pull/88)**、开放 issue 0。master 上复跑 `just ci` 全绿 **856 测试**（849 → 856，+7 为 #89 的守卫），远端 CI 针对 `e9220bf` 5 项全过。
  - **#88 的剩余工作两项**（PR 内容见下方 #88 两条专条）：
    - ① **`94b53aa`「补跑四视角审查——3 imp」自身从未过审**——249 行、动了 `validate/mod.rs`/`lib.rs`/`cli_e2e.rs`/`run.md`/`06`/`09`，按 CLAUDE.md 豁免表 4 视角一个都跳不掉。这不是新加的规矩，是 #88 自己记的教训（上一轮记「闭环」时，两个 imp 修复共 174 行未复审，补跑后三视角各自独立复现同一真实缺陷）。**第三轮审查进行中**。
    - ② **PR 当前 `CONFLICTING`**（`docs/STATUS.md` + `docs/design/06-plugin-structure.md` 与 #89 合并后的 master 冲突）。远端那个绿 CI 是 2026-08-08 针对 `94b53aa` 跑的，此后 master 前进了。须 rebase 后让远端 CI 针对**新 sha** 复跑，再记「待拍板」——#89 的教训：冲突 PR 上 GitHub 根本不跑 workflow，「no checks reported」会被误读成排队（当时白等 35 分钟），判据是 `gh pr view --json mergeable` 而非等 checks。
  - **审查轮次设界**：第三轮范围严格限定 `94b53aa` 的 diff；其修复产物若落在豁免阈值内（<30 行且不涉错误处理/新类型/测试）即直接收口，不起第四轮。（该设界随本轮计划一并向用户提出并获同意执行——「审查修复本身要再过一轮」这条规则自身没有终止条件，须显式设界。）
- **`blocked_by` 幽灵引用检出已交付（PR [#88](https://github.com/snowzhaozhj/rewriteInRust/pull/88) 第二部分，第三轮四视角审查进行中）**——收口 [MDR-021](decisions/021-no-json-schema-validation.md) 待办 1，是该批待办里唯一的**真实功能缺口**（其余为文档/守卫类）。`just ci` 全绿 **879** 测试（861 → 877 → 879，末次 +2 为补跑四视角后新增的 e2e）。**审查状态：四视角已两轮，第三轮进行中**——第一轮针对功能主体，第二轮针对第一轮的修复产物，**第三轮针对第二轮的修复产物 `94b53aa`**（249 行，超 CLAUDE.md 全部四项豁免阈值；详见下方两条教训与缺陷清单）。
