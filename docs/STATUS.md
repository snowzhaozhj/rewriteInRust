...
- **MDR-019 译后签批门落地（2026-07-27，PR [#84](https://github.com/snowzhaozhj/rewriteInRust/pull/84)，待用户验收）**：M4 收官后唯一「已决策、未落地」的设计偏离——`reviewing`「最终签批门」被实现成编排器自动 `--to done`。落地把门重建在 `reviewing → done` 这条边**本身**：
