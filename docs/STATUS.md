...
- **SKILL.md ↔ CLI 命令清单守卫 ✅ 交付**（2026-08-08，独立 PR，从 master 起）——收口 #86 记账 TODO ①。`06` 那边由 #86 钉住，但 `06:105` 表头同时要求同步 SKILL.md 清单、`SKILL.md:31` 自称「已穷举顶层子命令」，此前**零自动化检查**（#85 那次是一次性人工验证）。编排器直接读 SKILL.md，漂了就照抄不存在的命令。新增 4 测试（2 守卫 + 2 解析器合成字符串单测），追加进既有 `design_command_table.rs` 以复用 `cli_leaf_commands()`。
  - **格式与 06 表格不同故不能复用 `parse_design_table`**：清单是锚点行之后连续 8 个「`- **<分组>**：`cmd`、`cmd`…`」的行内反引号列表。
  - **#86 记账预警的两个坑，实测后一个成立、一个不成立**（按实测写，不照抄记账）：⒜ **成立**——文档别处也有同格式的 `- **X**：` 段落（`守护`/`恢复`/`幂等`/`待签批`），全文件按行格式匹配会一并收进来；**其中缩进两格、真能落进区块的是 3 个**（`L1 存在性`/`L2 结构校验`/`幂等`）。故须以「命令清单 + 已穷举」锚点行定位区块、按**缩进量**收尾（紧随其后就有一个同格式但缩进为零的 `profile --adapter-tools` 一级列表项）。**本条初版记述被设计契约审查订正**：原写「命中 13 行、5 个别处段落、会拖出大批伪幽灵」——13/5 取自未写明的更窄正则口径（按字面正则是 32 行），而「大批伪幽灵」实为坑⒝的后果（单独摘掉⒜不产生任何伪幽灵，只污染分组标签集合）。**在一个专门消灭「文档给出的具体值实现里查不出来」的 PR 批次里，我自己写了口径含糊的具体值**，已按实测订正。⒝ **不成立**——记账预警「`graph export [--format json|dot|mermaid]` 的 `json|dot|mermaid]` 会残留成命令名一部分」，实测该风险只存在于「正则替换占位符」的实现；**逐 token 在首个占位符处 break** 时管道段落在 break 之后、根本到不了。已在代码注释如实记录该实测结论。
  ...
  - **当下一致性 35/35**（守卫首跑即绿，与 #86 记账一致；**正因首跑即绿才必须做上述负向实证**）。`06:105`、`SKILL.md:31`、测试文件头三处标注「两侧均有 CI 守卫」。`just ci` 全绿。
