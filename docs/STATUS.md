...
- **进行中 PR [#86](https://github.com/snowzhaozhj/rewriteInRust/pull/86)（收口 #85 三项记账 TODO，2026-07-28，审查中）**：三项均为「说法与实现不符」，其中 ② 含破坏性变更。`just ci` 全绿 **814 测试**（基线 809 → +5）。
  ...
  - **撤回一项初判**：调查中曾疑 `WorkspaceConfig{}` 空结构与 06:841-843 的 `cargo_workspace`/`crate_naming` 不符，核实为**非偏差**——与 `ToolsConfig`/`ParserConfig`/`AnalysisConfig`/`ReproducibilityConfig`/`ValidationConfig` 同属既有「M2 预留」空结构模式。
