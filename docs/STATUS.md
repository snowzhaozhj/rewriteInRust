...
- **最新合并 PR**: [#66](https://github.com/snowzhaozhj/rewriteInRust/pull/66)（Sprint F ROB-01b watchdog stall 检测 + 恢复路径，**初版已合并**——4 视角审查修复在合并后补交，见下方「⚠️ 审查修复独立 PR」）；[#65](https://github.com/snowzhaozhj/rewriteInRust/pull/65)（ROB-01a）；[#64](https://github.com/snowzhaozhj/rewriteInRust/pull/64)（GOV-01）
...
- **⚠️ ROB-01b 审查修复独立 PR（合并后补交）**：#66 初版在 4 视角审查跑完前即被合并（merge commit `04611e8`，parent = 初版 `0665ccd`），**2 个 important 修复未进 master**（degrade 守护 bug + unblock_next 命令 bug）。审查修复（含上条 5 项）已 cherry-pick 到新分支 `fix/m4-rob-01b-post-merge-review-fixes` 独立提 PR，**待用户审阅合并（不自行 merge）**——合并后 master 的 ROB-01b 才完整。
- **开放 PR**: `fix/m4-rob-01b-post-merge-review-fixes`（ROB-01b 审查修复补交，见上）
