...
- **位置（2026-08-12，本条最新）**：master 在 `10005da`（#88 已合并 + STATUS 回填）。**进行中：处方层 PR**（分支 `feat/m4-state-repair-ghost-blocked-by`）——新增 `state repair --clear-ghost-blocked-by`，收口「幽灵引用检出已交付、处置为零」这个状态。决策与边界见 **[MDR-022](decisions/022-ghost-blocked-by-repair-boundary.md)**。
  ...
  - **三条不变量**：① 只删幽灵条目，`Resolved`（能归一到宿主模块的引用，与宿主是否终态无关）与 `Ambiguous`（宿主歧义——跨组 `member_files` 破坏的**唯一检出通道**）原样保留，不清整个数组；② 不改状态、不清进度字段、不发删产物指令（与 `reset` 正相反），恢复交既有 `--auto-unblock`；③ `RepairedModule` 只放字段不放谓词，锚点判定留在 CLI 层（塞复合谓词进数据结构正是前一轮出错的形状）。
  ...
  - **待拍板**：PR [#90](https://github.com/snowzhaozhj/rewriteInRust/pull/90)，**已推送 `f91a0e3`**（= 功能 `daf8c74` + 审查修复 `f91a0e3`），**远端 CI 针对该 sha 复跑 5 项全过**（check/coverage/deny/shellcheck/test），PR `MERGEABLE`；本地 `just ci` 在最终代码上全绿 **897 测试**。**负向实证累计 11 轮**（独立 worktree，全部报红且归因精确）——功能层 7 轮见 PR 正文，审查修复层 4 轮：⑧ 就绪判据退回 `blocked_by_remaining.is_empty()`（复现三视角抓到的漏报）→ 锚点测试红；⑨ 告警守卫恒真 → 同一测试红（合法锚点须静默那一侧）；⑩ noop 也落盘 → 幂等测试红（`.backup` 被覆盖）；⑪ noop 回显退回原始入参 → 幂等测试红。**绝不自行 merge**——等用户审阅拍板。
