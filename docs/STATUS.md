...
- **进行中 PR [#85](https://github.com/snowzhaozhj/rewriteInRust/pull/85)（文档/提示词与实现对齐，2026-07-28，待用户验收）**：收口三项 STATUS 已记账 TODO，均为「说法与实现不符」，不改行为。
  ...
  - **设计契约审查 3 项已修**：① 06 的 `state deps` 输出漏第 5 个字段 `unresolved`——它单列「依赖未登记为模块」且**不计入 `blocking`**（否则被 run 填进 `blocked_by` 造成永久 blocked 死锁），是门禁消费方必须知道的语义；② 同行「亦被 `state resume` 的 `next` 桶复用」失实——`cmd_state_resume` 不调用 deps 逻辑，只在 `advice` 字符串里指引编排器去调；③ `record-subagent-call` 的 `--status` 我写成「(started/ok/error)」像是枚举，实际 **CLI 不做枚举校验**（自由字符串），且附录 A 示例是 `success`、SKILL.md 台账约定用 `started`/`ok`/`error`——两套口径并存，改为如实记录并标注待统一。另修 `graduate` 的「幂等守护」用词（实为拒绝重入报错，非幂等）。
  - **本 PR 记账 TODO（既有偏差，不在本 PR 扩范围）**：① `scaffold workspace` 名为 workspace，实际 `cargo init --lib` 产出单 crate、无 `[workspace]` 段——命令名与产出物语义不符，改需同时动命令名与文档，独立评估；② `record-subagent-call` 的 `--status` 值域三方不一致（clap 帮助「success/timeout/failed」× 附录 A 示例 `success` × SKILL.md 约定 `started`/`ok`/`error`），CLI 无校验故不报错，宜定一套并加校验；③ 06 命令表的计数与命令列表靠人工维护、无自动化守卫（本 PR 的漂移即由此而来），宜加「06 表 ↔ CLI 叶子命令」一致性测试。
