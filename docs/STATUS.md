...
- **最新合并 PR**: [#65](https://github.com/snowzhaozhj/rewriteInRust/pull/65)（Sprint F ROB-01a checkpoint 硬化 + 幂等重试，已合并）；[#64](https://github.com/snowzhaozhj/rewriteInRust/pull/64)（Sprint F GOV-01 规则版本陈旧检测）；[#63](https://github.com/snowzhaozhj/rewriteInRust/pull/63)（M4 Sprint D PLG-03~06）
...
- **开放 PR**: [#66](https://github.com/snowzhaozhj/rewriteInRust/pull/66) Sprint F ROB-01b（watchdog stall 检测 + 恢复路径，分支 `feat/m4-sprint-f-rob-01b-watchdog-stall-recovery`）。**待用户审阅拍板合并（不自行 merge）**
