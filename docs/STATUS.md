...
- **进行中：PR「scaffold 父 workspace 告警」（收口 #86 记账 TODO ②，分支 `fix/m4-scaffold-parent-workspace-warning`，待审查）**：`scaffold workspace` 落在已有 `[workspace]` 的仓库内时，`cargo init` 把新 crate 追加进用户父 `Cargo.toml` 的 `members`，而 CLI 此前返回 `status:ok` **零 warning**——静默改了用户仓库的构建配置，此后其 `cargo build`/`test` 会连带编译迁移产物（迁移中的 crate 常是 `unimplemented!()` 等不可编译中间态，足以把原本绿的构建搞红）。
  - **检测判据 = 改动前后比对父 manifest 内容**，不匹配 `cargo init` 的 stderr 文案（`Adding ... as member of workspace` 随 cargo 版本变动且可能本地化）。`find_enclosing_workspace_manifest` 自目标目录**父目录**起沿 ancestors 找第一个含 `[workspace]` 的 `Cargo.toml`（跳过目标自身——那是本次要生成的产物），只做词法判断不解析 TOML（目的是「能否比对出改动」，宁可多找候选不该因 TOML 方言漏检）。
  - **处置 = 告警不报错**：追加 member 本身无破坏、用户也可能确实想要（迁进 workspace 是合理意图），故汇入 `warnings` + 降级 `status=warning` + 给出被改文件路径，能否接受由用户判断；CLI 的职责是不让它静默发生。两个 scaffold 函数（`scaffold_project` / `scaffold_project_with_bin`）**均接线**，签名 `Result<()>` → `Result<Vec<String>>`（调用面仅 CLI 一处，`with_bin` 除测试零调用点）。幂等重跑走早返回路径不调 `cargo init`，故不重复告警（否则编排器每次重入收到无动作可做的噪声）。
  - **测试 +8（828 → 836）**：core 7 单测（父 workspace 告警含「先证 cargo 真改了父 manifest」的前置假设断言防空转 / 裸目录不告警 / 父为普通 `[package]` 不告警且不被改 / `with_bin` 同样告警 / 幂等重跑不重复告警 / 查找函数跳过目标自身 manifest / 多级嵌套找到祖辈）+ 1 CLI e2e（专证 warnings 真的接进统一 JSON 并降级 status——core 单测只覆盖逻辑，两者缺一用户都看不到）。**负向实证两轮**：① 让检测恒返回空 → 3 测试报红；② 只摘掉 `with_bin` 一处检测 → 对应测试独立报红（证明两函数各有守卫、不互相掩盖）。此前全部 scaffold 测试都在裸 tempdir 跑，这条路径零覆盖。
  - **文档同步**：06 命令表该行补检测/处置口径；`scaffolder.md` R1 补护栏——**agent 不得自行编辑用户的父 `Cargo.toml`**（无论移除条目还是加 `exclude`），那是用户仓库的构建配置、改法取决于其意图，照原样转达告警即可。
  - `just ci` 全绿（836 测试 + fmt + clippy + deny + shellcheck）。
