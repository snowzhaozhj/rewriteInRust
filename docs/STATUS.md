...
- **最新合并 PR**: [#68](https://github.com/snowzhaozhj/rewriteInRust/pull/68)（ROB-01c 额度续跑）；[#67](https://github.com/snowzhaozhj/rewriteInRust/pull/67)（ROB-01b 4 视角审查修复补交，ROB-01b 至此完整）；[#66](https://github.com/snowzhaozhj/rewriteInRust/pull/66)（Sprint F ROB-01b watchdog stall 检测 + 恢复路径初版）；[#65](https://github.com/snowzhaozhj/rewriteInRust/pull/65)（ROB-01a）；[#64](https://github.com/snowzhaozhj/rewriteInRust/pull/64)（GOV-01）
...
- **Sprint F ORCH-01 决策反转**（2026-07-14，分支 `docs/m4-orch-01-keep-parallel`，本 PR）：先前一版「砍并行、回归串行」的决策（原 MDR-018 串行版，PR #69）**经用户否决并撤销**——并行是本项目核心能力，砍则失去意义。串行化论据的错误在于**偷换「并行翻译」与「并行审批」**：并行翻译独立模块只把待审队列填快，审批仍由人从队列串行取（[MDR-019](decisions/019-post-translation-review-gate.md) 译后签批门保证），二者正交。**PR #69 已 reset 回 master + 关闭**、错误分支删净。本 PR = 纯决策文档：新 **[MDR-018](decisions/018-keep-parallel-migration.md)**（保留并行 + worktree 写隔离 + 串行审批三者共存；重申 [MDR-003](decisions/003-m2-parallel-write-isolation.md) 有效）+ 重写 [MDR-019](decisions/019-post-translation-review-gate.md)（并行前提下签批门是二者共存机制）+ PLAN-M4 纠正「并行已落地」为 coded-but-dead 并重定义 ORCH-01。**关键事实**：调查确认并行/worktree 基础设施当前仅 **coded-but-dead**（`parallel.rs` 死类型 / `parallel_groups` 不落盘 / `max_concurrent_agents` 死字段 / 两套 worktree 机制矛盾 / 无 E2E），从未跑通——「保留并行」= ORCH-01 须**真正把它建起来并跑通**（实现任务，单独 Sprint 规划估时，非本 PR）。
- **开放 PR**: `docs/m4-orch-01-keep-parallel`（ORCH-01 决策反转：保留并行 + MDR-018/019，待审查）
