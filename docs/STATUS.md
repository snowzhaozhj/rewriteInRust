...
- **Sprint B 进行中（分支 `feat/m2-sprint-b`，未提 PR）—— Wave 1 已完成 6/7**：执行模式按热点文件分组（非朴素 4 路并行）：隔离文件走 worktree subAgent 并行，热点簇（graph.rs/build.rs/persist.rs/typescript.rs）由主会话串行做避免合并地狱
  - ✅ **REFAC-12**(merge `8229851`，worktree agent): extract_class 递归改走完整 `walk_ast`（原 `walk_ast_calls_only` 删，成死代码）→ 嵌套 class 方法节点 + 内部 dynamic import 健壮捕获；新增回归测试
  - ✅ **REFAC-15**(merge `1890e95`，worktree agent): `humanize_module_key` 纯函数 + `state get --human` 标志输出 `data.human` 友好名（内部 key 不变、向后兼容）。**触 CLI JSON 契约**（`--human`/`human` 字段）→ **提 PR 前须挂 design-checker**，可能需在 06 设计文档登记
  - ✅ **REFAC-03+04**(`8825e61`，合批): `sub_kind`→`EdgeSubKind`(Implements/Constructor)、`migration_status`→`Option<ModuleStatus>`(复用 state 层枚举)、`rust_kind`→`Option<RustKind>`(PascalCase 对齐既有落库)。落库走 Display/parse 同 NodeType 模式；非法值按 complexity 静默 None。对应 04 §5.7.1
  - ✅ **REFAC-11**(`ae76ca3`): fixup_extends 预建名称→Interface/Class/Enum 索引 HashMap，跨文件唯一命中查找 O(N·E)→O(1)；语义不变；find_unique_node 保留供构造调用兜底
  - ✅ **REFAC-02**(`8abcd45`): ImportInfo 三布尔→`ImportKind`(StaticValue/StaticType/SideEffect/Dynamic)、ImportedSymbol 双布尔→`SymbolKind`(Named/Default/Namespace)，编译期消除非法组合。**ImportInfo 系内部中间类型无契约面**；PLAN 标注的 build.rs 实际不消费这些字段（PLAN 不精确）
  - 质量门：每任务独立验证，整批 `just ci` 全过（234 测试 / clippy -D / deny / fmt / shellcheck）。toml duplicate 系 tokei 既有传递依赖
- **下一步**（**新会话从这里开始**）: **REFAC-01 + Wave 2（09→10）**，仍在 `feat/m2-sprint-b`
  - **REFAC-01 调查结论（重要，避免重蹈 REFAC-06/13 过度封装）**：原任务「构造器 + **pub(crate)** 封装」——`pub(crate)` **不可行**：`cli/lib.rs:754,760` 跨 crate 直读 `n.is_exported`/`n.node_type`，私有化会破坏 cli 或迫使加被 B 档否决的 getter 墙。**收缩为**：只加 `SourceNode::new(id,node_type,name,file_path)` 构造器默认其余可选字段（收敛全仓 **18 个**冗长 struct 字面量、funnel 默认值），**字段保持 pub**（纯数据 struct + cli 跨 crate 读）。RustTarget 字段不变量（rust_kind/rust_path/crate_name 仅 RustTarget）可加 `rust_target(...)` 构造器，但这些字段当前 **预留未产出**（04 §5.7.1，graph build 不写）→ 强制不变量属投机，**暂缓**
  - **Wave 2 串行**：REFAC-09（arrow function 提取 `export const f = () => {}`→Function 节点，依赖 REFAC-01，typescript.rs）→ REFAC-10（跨文件方法调用档1，graph/build.rs，3-4d 单项最大）
  - 注：M2-TIER-01a 删 risk 时需同步 plugin 提示词 analyze.md:37 的 `risk:low` 表述。**PR 粒度已放宽**：Sprint B 拟整批一个 PR
  - **提 PR 前审查路由**：`/code-review`(必) + REFAC-15 触契约挂 `design-checker` + REFAC-03/04 enum 触 types 字段挂 `design-checker`
