...
- **Milestone**: M1 ✅ → M2 ✅ → **M3 ✅** → **M3 遗留债清理 ✅** → **M4「完善」执行中——巩固线 Sprint A ✅（PR #57）+ Sprint B ✅（PR #58）→ Go 线 Sprint C ✅ 全合并（C1 #59 / C2 #60 / C3 #61）→ Sprint D「Plugin Go 适配」✅ 全合并（#62 + #63）→ 巩固线 Sprint F「健壮性+编排收口」启动：GOV-01 规则版本陈旧检测 ✅ 交付 + 4 视角审查全跑、2 important + nit 修复、`just ci` 全绿（分支 `feat/m4-sprint-f-gov-01-rule-version-check`，待用户验收）**
...
- **最新合并 PR**: [#67](https://github.com/snowzhaozhj/rewriteInRust/pull/67)（ROB-01b 4 视角审查修复补交，ROB-01b 至此完整）；[#66](https://github.com/snowzhaozhj/rewriteInRust/pull/66)（Sprint F ROB-01b watchdog stall 检测 + 恢复路径初版）；[#65](https://github.com/snowzhaozhj/rewriteInRust/pull/65)（ROB-01a）；[#64](https://github.com/snowzhaozhj/rewriteInRust/pull/64)（GOV-01）
...
- **Sprint F ORCH-01（重定义）决策就绪**（2026-07-05，分支 `docs/m4-orch-01-serial-migration-decision`，本 PR）：ORCH-01 原「worktree 生命周期代码化」前提被推翻——用户拍板**砍并行、砍 worktree 隔离、迁移工作流永久回归串行**（迁移是终端用户逐模块 review 的 human-in-the-loop 过程，并行破坏可审性、速度非卖点、并行隔离成果回收从未实现属坏投资）。ORCH-01 重定义为**工作流串行化收口**。**本 PR = 决策/方案层**：新增薄决策 **[MDR-018](decisions/018-serial-migration-drop-parallel.md)**（含下次开工的文件级落地清单）+ 取代 [MDR-003](decisions/003-m2-parallel-write-isolation.md)（加取代横幅）+ 删除虚构的大 ADR 草稿（`docs/design/decisions/`，编号/取代对象均系虚构）+ PLAN-M4 重定义 ORCH-01 并修正「并行已落地」表述。**实际收口**（CLI 死代码删除 + plugin 文档串行化 + 设计文档一致化）留到下次开工，照 MDR-018 落地清单执行（~1.5d）。调研已确认 serde 兼容安全（无 `deny_unknown_fields`、`MigrationSequence` 不反序列化）。
- **开放 PR**: `docs/m4-orch-01-serial-migration-decision`（ORCH-01 串行化决策 + MDR-018，待审查）
