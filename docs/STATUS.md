...
- ✅ **M2-REFAC-06 + REFAC-14 + REFAC-13**（分支 `feat/m2-refac-06`，B 档「封装类」批，3 commit）——**REFAC-06**(`873ec98`): MigrationSequence order/parallel_groups/cycles 私有化+只读 getter（为 SCALE-P 提供稳定入口，三者同源于一次拓扑计算）；**REFAC-14**(`d09109b`): ErrorData 加 `details: Option<Value>`，`#[serde(flatten)]` 让键提升到 `data` 顶层——`data.cycle_path` 路径保持不变（对齐 09 § Step 2.8 + analyze.md），cmd_graph_topo 环分支消除手搓 json! 走统一 ErrorData；**REFAC-13**(`501b446`): ToolStatus 探测三态收敛为私有枚举 `ToolProbe{Missing|ProbeFailed|Available}`（消除非法字段组合）+ getter，自定义 Serialize 摊平回历史扁平 JSON（tool_checks 契约不变），LocReport totals 改私有派生（from_languages 累加，JSON 字段名不变）。211 测试全过、clippy 零。**注**：REFAC-13 PLAN 标 types/state.rs 系笔误，实际 profile/tools.rs。**待跑 /gate + 提 PR**
