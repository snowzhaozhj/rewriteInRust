...
| # | 缺口 | 现象 | 建议 |
|---|------|------|------|
| 1 | **scaffold workspace 不生成 `.gitignore`** | worktree 内 translator 自检产生的 `target/` 被 `git add -A` 吞进提交，merge 时污染（每路 30+ target 文件）+ 前置层提交也被污染（主分支 31 个 target 文件） | `scaffold::workspace` 生成 crate 时补 `.gitignore`（含 `/target`）——Rust 项目标配。**已提 TODO** |
