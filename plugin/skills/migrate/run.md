...
当 sprint 内同拓扑层有多个独立模块时，编排器可并行派发翻译任务（`max_concurrent_agents` 由 `.rustmigrate.toml` 配置，默认 3）。

### 通信协议（7 步）

1. **派发（编排器 → SubAgent）**：编排器为每个目标模块 `git worktree add`（从主 HEAD，含全部 done 代码），派发 `TranslationDispatch`（module_key + worktree_path + dependency_interfaces + porting_rules）。porting 规则约束包要求：优先用既有共享 API；不够时用 `Error::Other`/`anyhow` 逃生口；禁删/改签名既有共享 API，新增只 append。
2. **翻译+自检（worktree 内隔离）**：SubAgent 在自己 worktree 内执行完整 M1 翻译循环（translate → cargo check → compile_fix → test），保留 per-module 编译反馈环。
...
4. **合并（编排器，结构化为主）**：own 文件复制进主 rust_root（同名/布局碰撞当冲突）；Cargo.toml/lib.rs 结构化 union（toml_edit / mod 追加）；其余共享 .rs 用 git merge。
5. **reconcile（仅 git 冲突时）**：冲突模块按依赖序逐个 worktree rebase 到已合并主线重译（非 LLM 手解冲突块）。轮次上限默认 3，超限降级串行/转人工。
6. **真门（整组 check）**：merge 后整组 `cargo check` / `cargo test`。通过 → `agent_done` 升 `done`；不通过 → compile_fixing 子流程。**这是唯一 done 真门**。
...
编排器在所有并行 agent 回传 `agent_done` 后，执行结构化合并（own 文件复制、Cargo.toml/lib.rs 结构化 union、共享 .rs git merge），然后在主 worktree 上执行整组验证：
...
- 并行模式由 `/migrate workflow`（M2-SCALE-01）调度，自动启用 §6-§7。
