...
- **幂等**：模块已在干净入口（`translating`/`pending` 且进度字段皆空）时为空操作（`was_noop=true`，不追加审计、免落盘）——`reset` 可安全重复调用。
- **守护**：`done`（终态）/ `blocked`（依赖锚点）/ `degrade_*`（降级须人类确认）须显式 `--force`，否则报错。
- **产物清理归编排器**：CLI 不删 `rust_root` 下的 `.rs`（不猜路径）。`reset` 输出 `data.cleanup.member_files`（模块源作用域）——据此删这些源文件对应的部分 `.rs` 产物后再 `run <M> --retry`；`intermediate/attempts/*` 与审计产物保留。
