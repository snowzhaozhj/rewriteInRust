...
- 命令清单（**已穷举顶层子命令**——CI 守卫双向钉住**子命令路径**这一层：新增命令未登记、或列出 CLI 里不存在的命令都会让测试报红，连顶层名就不对的项也会被当「未分类项」报出。**参数不在守卫范围内**、亦非穷举，照抄本清单里的参数不保证能解析，细节以各子命令 `--help` 为准）：
  ...
  - **断点续跑与数据修复（ROB-01a/b/c + MDR-022）**：`state reset --module <m> [--force]`、`state recover --module <m> --policy <retry|skip> [--reason]`（`--module`/`--policy` 均必填）、`state resume`（无参数）、`state repair --clear-ghost-blocked-by [--module <m>]`（删除无处归属的 blocked_by 引用；修复类型 flag 必填、`--module` 省略即清全部模块；只删幽灵条目，不改状态不清进度）
