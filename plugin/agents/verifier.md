...
| 5 | 字符串 | **Go `string` 是只读字节序列（通常 UTF-8 编码，但不保证合法 UTF-8），`len(s)` 为字节数——与 Rust `.len()` 语义一致**（这是 Go→Rust 的天然优势，不同于 TS UTF-16 / Python 码点）；但 `for i, r := range s` 按 rune（码点）迭代、同时给出字节索引 `i` ≈ Rust `.char_indices()`，`s[i]` 是**字节索引**（返 `byte`）≈ Rust `s.as_bytes()[i]`，`[]rune(s)` 转码点数组——验索引/迭代按 byte-vs-rune 语义选对，别把 rune 迭代错译成字节索引；源串非法 UTF-8 时 Rust `String`/`&str` 会拒绝，需按 `Vec<u8>`/`bytes` 处理 |
| 6 | 并发 | **goroutine 泄漏**：goroutine 无 owner、退出静默，永久阻塞的 goroutine 及其栈**不被 GC 回收**（只有进程退出才释放），源码可能默认靠进程退出兜底；Rust `task`/`thread` 须显式 join/管理生命周期——验是否补了取消/join。**`defer` vs GC 时机**：Go `defer` 是**确定性 LIFO** 释放（锁/文件/连接），GC 回收非确定性——验 `defer` 清理是否用 Rust `Drop`/RAII/`scopeguard` 还原确定性释放顺序，而非依赖 drop 时机漂移 |
| 7 | 错误路径 | `if err != nil` 逐层上抛 → 验 Rust 是否用 `Result` + `?` 还原同一传播链而非 `unwrap()`。**`recover()` 边界**：只在 `defer` 中捕获**同一 goroutine** 的 `panic`，跨 goroutine panic 不可恢复直接 crash——按 `adapters/go/porting-template.md` RULE-3，验 translator 是否把 `recover` 映射为 `Result` 显式传播（跨 goroutine/线程 panic 走 `JoinHandle::join` 的 `Err`），**而非用 `catch_unwind` 模拟 recover**（除非确属不可恢复的边界隔离），跨线程 panic 边界未混淆 |
